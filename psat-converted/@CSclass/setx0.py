...
# Source: third-party/psat/@CSclass\setx0.m  (upstream PSAT, GPL-2.0+)
...
# A
a.dat(:,7) = np.pi*a.con(:,14).*a.con(:,14)
...
# first guess
  ...
  while max(abs(inc)) > 1e-5
    ...
    if iter > 20
      fm_print(['Initialization of constant speed wind turbine #',...
...
if not check
  fm_print('Constant speed wind turbine cannot be properly initialized.')
else
  fm_print(['Initialization of constant speed wind turbines completed.'])
