...
# Source: third-party/psat/fm_input.m  (upstream PSAT, GPL-2.0+)
...
if nargin==5  and  isstruct(Resize),
...
[rw,cl]=size(NumLines)
OneVect = np.ones((NumQuest,1))
if (rw == 1  and  cl == 2)
  NumLines=NumLines(OneVect,:)
elseif (rw == 1  and  cl == 1)
  NumLines=NumLines(OneVect)
elseif (rw == 1  and  cl == NumQuest)
  NumLines = NumLines';
elseif rw != NumQuest  or  cl > 2,
  error('NumLines size is incorrect.')

if not iscell(DefAns),
...
QuestYOffset=np.zeros((NumQuest,1))
EditYOffset=np.zeros((NumQuest,1))
...
for lp=1:NumQuest,
  ...
  if size(NumLines,2) == 2,
    set(EditHandle(lp),'String',char(np.ones((1,NumLines(lp,2))*'x'))
...
# Determine the height of all question fields
  ...
  Num1Liners = len(find(Data.LineInfo(:,1)==1))
...
# Loop over each quest/text pair
  ...
  for lp = 1:len(Data.QuestHandles),
