...
# Source: third-party/psat/fm_main.m  (upstream PSAT, GPL-2.0+)
...
if nargin
  if not exist('Fig'), Fig.main = -1, end
  if not ishandle(Fig.main)
  ...
  switch varargin{1}
   ...
   case 'buttondown'
    ...
    if pos(1) > 0.04064  and  pos(2) > 0.1152  and  ...
          pos(1) < 0.2943  and  pos(2) < 0.3395
      if not exist('Hdl'), return, end
      if ishandle(Hdl.status)
        delete(Hdl.status)
        Hdl.status = -1
      elseif strcmp(get(Fig.main,'SelectionType'),'open')
        fm_about
    elseif pos(1) > 0.0406404  and  pos(2) > 0.0358255  and  ...
          pos(1) < 0.9618  and  pos(2) < 0.0794
      if strcmp(get(Fig.main,'SelectionType'),'open')
        fm_hist
    elseif pos(1) > 0.098522  and  pos(2) > 0.7180  and  ...
          pos(1) <  0.5751  and  pos(2) < 0.7569
      if strcmp(get(Fig.main,'SelectionType'),'open')
        fm_set setpert,
    elseif pos(1) > 0.098522  and  pos(2) > 0.8317  and  ...
          pos(1) < 0.5751  and  pos(2) < 0.8706
      if strcmp(get(Fig.main,'SelectionType'),'open')
        fm_set setdata,
...
h3 = uimenu('Parent',h2, ...
            ...
            'Tag','SaveS')
if not allowsimconv, set(h3,'Enable','off'), end
h3 = uimenu('Parent',h2, ...
            'Callback','fm_simsave', ...
...
h1 = uicontrol('Parent',h0, ...
               'Units','normalized', ...
               'BackgroundColor',Theme.color04, ...
               'CreateFcn','if ~isempty(File.pert)  and  ~strcmp(File.pert,''pert''), set(gcbo,''String'',File.pert); end', ...
