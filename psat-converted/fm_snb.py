...
# Source: third-party/psat/fm_snb.m  (upstream PSAT, GPL-2.0+)
...
if not autorun('SNB Direct Method',0), return, end
...
if not CPF.init  and  not clpsat.init
  Settings.ok = 0
...
if ncload  or  DAE.n
  fm_print('only PV, PQ and SW buses are allowed for SNB computations.')
  return

fm_disp
if distr
  fm_print('Direct Method for SNB Computation - Distribuited Slack Bus')
else
  fm_print('Direct Method for SNB Computation - Single Slack Bus')
fm_print('                                  - Right Eigenvector')
fm_disp
...
if not distr, SW = setgamma(SW); end
...
[Vect,D] = np.linalg.eig(full(DAE.Gy))
diagD = diag(D)
idx = find(diagD == 1)
if not isempty(idx)
  diagD(idx) = 1e6
[val,idx] = min(abs(diagD))
Vect = np.linalg.inv(Vect.');
...
# non-trivial condition  or w or  = 1
  norm_eq = np.linalg.norm([M',Z]);
  Vjac = [M',Z]/norm_eq;
...
# complete Jacobian matrix
  ...
  if Settings.error > 2*err_max_old  and  iteration
    ...
    if Settings.show
      fm_print(['iteration = ', int2str(iteration), ...
...
Settings.lftime = toc
if robust < tol
  fm_print('Increment step below threshold. Bad initial point. SNB routine stopped.',2)
elseif iteration >= iter_max
  fm_print(['Direct Method for SNB did not converge.'],2)
else
  fm_print(['Direct Method for SNB completed in ',num2str(toc),' s'])
