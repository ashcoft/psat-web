...
# Source: third-party/psat/fm_opfsdr.m  (upstream PSAT, GPL-2.0+)
...
if not autorun('Optimal Power Flow',0)
  return

if DAE.n
  fm_print('OPF routine does not support dynamic components',2)
  return

if not Supply.n,
  fm_print('Supply data have to be specified before in order to run OPF routines',2)
  return
...
w = OPF.w
if w > 1  or  w < 0
  fm_print('The weigthing factor range is [0,1]')
  return

if OPF.show
  fm_disp
  fm_print('----------------------------------------------------------------')
  fm_print('Interior Point Method for OPF Computation')
  fm_print('Spot Price of Security (including loading parameter)')
  if w > 0  and  w < 1,
    fm_print('Multi-Objective Function')
  elseif w == 0,
    fm_print('Social Benefit Objective Function')
  elseif w == 1,
    fm_print('Maximization of the Distance to Collapse')
  fm_print(['Minimum Loading Parameter = ',num2str(lcmin)])
  fm_print(['Weighting Factor = ',num2str(w)])
...
if OPF.enreac
  ...
  Qgmin = -99*Settings.mva*np.ones((n_gen,1))
  Qgmax =  99*Settings.mva*np.ones((n_gen,1))

if OPF.envolt
  [Vmax,Vmin] = fm_vlim(OPF.vmax,OPF.vmin)
else
  Vmin = OPF.vmin*getnp.ones((Bus))
  Vmax = OPF.vmax*getnp.ones((Bus))
Vcmin = Vmin
Vcmax = Vmax
...
if OPF.enflow
  ...
  Iijmax = 999*999*np.ones((Line.n,1))
Iijcmax = Iijmax
...
ro = np.zeros((n4,1);     #  Dual Variables [ro])
...
g_Qg = sparse(n1+busG,nG,-np.ones((n_gen,1),n2,n_gen)
g_Ps = sparse(Supply.bus,nS,-np.ones((Supply.n,1),n2,Supply.n)
g_Pd = sparse(Demand.bus,nD,np.ones((Demand.n,1),n2,Demand.n)
g_Pd = g_Pd + sparse(Demand.vbus,nD,qonp,n2,Demand.n)
g_Qgc = sparse(n1+busG,nG,-np.ones((n_gen,1),n2,n_gen)
g_Psc = sparse(n2,Supply.n)
g_Pdc = sparse(n2,Demand.n)
DAE.Gl = np.zeros((n2,1))
DAE.Gk = np.zeros((n2,1))
dF_dy = sparse(n_y,1)
...
if not isempty(a)  or  not isempty(b)
  fm_print('Actual line flows are greater than imposed limits',1)
  fm_print(['Check limits of the following lines:'],1)
  fm_print('          from     to',1)
for i in range(1, len(a)+1):
    fm_print(['    Line ',fvar(Line.fr(a(i)),5), ...
             ' -> ', fvar(Line.to(a(i)),5),': ',fvar(Iij(a(i)),7), ...
             ' > ', fvar(sqrt(Iijmax(a(i))),7)],1)
for i in range(1, len(b)+1):
    fm_print(['    Line ',fvar(Line.to(b(i)),5), ...
...
b = find(Vbus < Vmin)
if not isempty(a),
  fm_print(['Max Voltage limit not respected at buses: ',num2str(a')],1)
for i in range(1, len(a)+1):
    fm_print(['     Bus #',fvar(Bus.con(a(i),1),4),' -> ',fvar(Vbus(a(i)),8), ...
             ' > ', fvar(Vmax(a(i)),8)],1)
if not isempty(b),
  fm_print(['Min Voltage limit not respected at buses: ',num2str(b')],1)
for i in range(1, len(b)+1):
    fm_print(['     Bus #',fvar(Bus.con(b(i),1),4),' -> ',fvar(Vbus(b(i)),8), ...
             ' < ', fvar(Vmin(b(i)),8)],1)
if not isempty(a)  or  not isempty(b)
...
if not isempty(idx),
  s(idx) = 1e-6
  fm_print('Problems in initializing slack variables...')
...
# reference angle for the actual system
    Jd(SW.refbus,:) = 0
    Jd(:,SW.refbus) = 0
    Jd(SW.refbus,SW.refbus) = spnp.eye(len(SW.refbus))
    gy(SW.refbus) = 0
# reference angle for the critical system
    Jd(:,SW.refbus+n2+n_a) = 0
    Jd(SW.refbus+n2+n_a,:) = 0
    Jd(SW.refbus+n2+n_a,SW.refbus+n2+n_a) = spnp.eye(len(SW.refbus))
...
#________________________________________________________________________
  ...
  if OPF.show
    fm_print(['Iter. =',fvar(iteration,5),'  mu =', fvar(ms,8), ...
...
if (Settings.showlf  or  OPF.show)  and  clpsat.showopf
  ...
  if not noDem
    OPF.report{6,1} = ['Total demand = ', ...
                        fvar(sum(Pd),8),' [p.u.]']
  OPF.report{6+(not noDem),1} = ['TTL = ',fvar(sum(Pd)+TPQ,8),' [p.u.]']
  ...
  if iteration > iter_max
    fm_print('IPM-OPF: Method did not Converge',2)
  elseif ishandle(Fig.main)
    if not get(Fig.main,'UserData')
      fm_print('IPM-OPF: Interrupted',2)
    else
      fm_print(['IPM-OPF completed in ',num2str(toc),' s (omega = ',num2str(w),')'],1)
  else
    fm_print(['IPM-OPF completed in ',num2str(toc),' s (omega = ',num2str(w),')'],1)
