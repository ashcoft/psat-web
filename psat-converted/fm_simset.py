...
# Source: third-party/psat/fm_simset.m  (upstream PSAT, GPL-2.0+)
...
if nargin
  if not exist('Fig'), Fig.simset = -1, end
  if not ishandle(Fig.simset)
    Fig.simset = findobj(get(0,'Children'),'FileName','fm_simset')
  switch varargin{1}

   case 'noarrows'

    Settings.noarrows = not Settings.noarrows
   ...
   case 'buttondown'
    ...
    if pos(1) > 0.118  and  pos(2) > 0.35  and  pos(1) < 0.318  and  pos(2) < 0.9
