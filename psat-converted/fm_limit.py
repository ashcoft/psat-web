...
# Source: third-party/psat/fm_limit.m  (upstream PSAT, GPL-2.0+)
...
if not autorun('LIB Direct Method',0), return, end
...
if ncload  or  DAE.n
  fm_print('only PV, PQ and SW buses are allowed for LIB computations.')
  return

fm_print('  ',1)
fm_print('Newton-Rapshon Method for LIB Computation - Distribuited Slack Bus',1)
...
if bus_no > Bus.n
  fm_print('Bus_no exceeds bus number',2)
  failed = 1
if bus_no < 1
  fm_print('Bus_no should be an integer > 0',2)
  failed = 1
...
switch type
 case 1
  a = findbus(PQ,bus_no)
  if isempty(a)
    fm_print('No PQ load found for the specified bus number',2)
    failed = 1
  eta = Vmax(a)
 case 2
  a = findbus(PQ,bus_no)
 ...
 case 3
  ...
  if isempty(a)  and  isempty(b),
    fm_print('No generator found for the specified bus number',2)
    failed = 1
  if a
 ...
 case 4
  ...
  if b
    ...
    SW = add(SW,move2sw(PV))
 otherwise
  fm_print('ERROR: option "',num2str(type),'" is not defined.',2)
  failed = 1
...
# call components functions and models
  ...
  fm_print(['iteration = ',int2str(iterazione), ...
           '    lambda = ',num2str(lambda), ...
           '    kg = ',num2str(kg), ...
           '    err = ',num2str(Settings.error)],1)

fm_disp
fm_print(['lambda critical = ',num2str(lambda)])
...
Dxf2c = Dxf1c
Dxf2c(bus_no,:) = np.zeros((1,DAE.m+1))
Dxf2c(:,bus_no) = np.zeros((DAE.m+1,1))
...
if iterazione >= iter_max
  fm_print(['Reached Maximum Number of Iterations for ', ...
           'LIB computation without Convergence'],2)
else
  fm_print(['Limit Induced Bifurcation computed in ', ...
