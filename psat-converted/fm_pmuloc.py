...
# Source: third-party/psat/fm_pmuloc.m  (upstream PSAT, GPL-2.0+)
...
if not Line.n
  fm_print('No static network is loaded. PMU placement interrupted.')
  return
...
tic
  ...
  fm_print(' ',1)
  fm_print('PMU Placement Routine',1)
  fm_print(['Data File "',Path.data,File.data,'"'],1)
...
for i in range(1, Bus.n+1):
  ...
  switch type
   ...
   case 1  #  depth first method
    ...
    metodo = 'Depth First'
    fm_print([metodo, ' Method '],1)

    while len(n_link(:,1)) > 0
# locazione del PMU nel nodo non osservabile pi interconnesso
      pmu_con = [pmu_con; n_link(len(n_link(:,1)),1)]
      i_idx = fm_iidx(pmu_con(len(pmu_con)),linee)
      I_idx = [I_idx; i_idx]
# nodi osservabili dal primo PMU
      nodi_oss = [i_idx(:,4);pmu_con(len(pmu_con))]
      a = []
for i in range(1, len(nodi_oss)+1):
        a = [a; find(n_link(:,1) == nodi_oss(i))]
      if not isempty(a); n_link(a,:) = []; end
      if ishandle(Fig.pmu)
        set(hdl_pmu,'String',int2str(len(pmu_con)))
        set(hdl_nob,'String',int2str(len(n_link(:,1))))
        drawnow
    pmu_test = pmu_con
...
#for ii = 1:length(nodi)
   ...
   case 5
    ...
    fm_print([metodo, ' Method '],1)
    pesi = np.zeros((1,Bus.n))
    coll = np.zeros((1,Bus.n))
...
for i in range(1, len(coll1_idx)+1):
      pmu_idx = find(A(coll1_idx(i),:))
      pmu_test = [pmu_test, getidx(Bus,pmu_idx)]
      if ishandle(Fig.pmu)
        set(hdl_pmu,'String',int2str(len(pmu_test)))
        set(hdl_nob,'String',int2str(len(find(pesi == 0))))
        drawnow
      pesi(pmu_idx) = 100
      pmu_idx = find(A(pmu_idx,:))
      pesi(pmu_idx) = pesi(pmu_idx) + np.ones((1,len(pmu_idx)))
...
# posizionamento dei PMU, nodi ad interconnessione maggiore di 1
    ...
    while 1
      n_pmu = len(pmu_test)
      basta = 1
      coll_idx = find(coll == j  and  pesi == 0)
for i in range(1, len(coll_idx)+1):
        pmu_idx = find(A(coll_idx(i),:))
        a = find(pesi(pmu_idx) >= 1)
        b = find(pesi(pmu_idx) == 0)
        if len(b) == 1  and  isempty(find(pesi(pmu_idx(a)) > 99))
          basta = 0
          break
      if basta,
        j = j + 1
        ...
        pesi(pmu_idx(a)) = pesi(pmu_idx(a)) + np.ones((1,len(pmu_idx(a))))
        pesi(pmu_idx(b)) = 100
        ...
        pesi(pmu_idx2) = pesi(pmu_idx2) + np.ones((1,len(pmu_idx2)))
      if n_pmu < len(pmu_test), j = 2; end
...
# assegnazione di un PMU ai nodi non ancora osservabili
    ...
    pmuloc = np.zeros((1,Bus.n))
    pmuloc(Bus.int(pmu_test)) = 1; # np.ones((1,pmunum))
    pmunum = len(find(pmuloc))
    [pmu_test, pmu_test2, I_idx, pseudoi, index_pmu, pmunum] = ...
        fm_spantree(zeroinj, pmuloc, pmunum, hdl_pmu, hdl_nob)
...
for i in range(1, Bus.n+1):
      ...
      while not isempty(find(not bus2))
        buspmu_new = []
for j in range(1, len(bus_pmu)+1):
          [buspmu, bus2]= fm_pmurec(bus_pmu(j),bus2,bus0)
          buspmu_new = [buspmu_new; buspmu]
          pmu_test = [pmu_test; buspmu]
        bus_pmu = buspmu_new
      pmu_test = sort(pmu_test)
      num_bus = len(pmu_test)
      nodi_el = []
      antenne = []
for jjj in range(1, len(pmu_test)+1):
        nodi_el = [nodi_el; jjj+find(pmu_test([jjj+1:len(pmu_test)]) ...
                                     == pmu_test(jjj))]

      pmu_test(nodi_el) = []
for jjj in range(1, len(pmu_test)+1):
        a = len(busidx(Line,pmu_test(jjj)))
        if a == 1, antenne = [antenne, jjj]; end
      pmu_test(antenne) = []
for jjj in range(1, Bus.n+1):
        if isempty(find(pmu_test == getidx(Bus,jjj)))
          a = busidx(Line,getidx(Bus,jjj))
          if len(a) > 1
            a_idx = []
for kkk in range(1, len(a)+1):
              a_idx = [a_idx; find(pmu_test == a(kkk))]
            if len(a_idx) < 2, pmu_test = [pmu_test; a]; end
      nodi_el = []
for jjj in range(1, len(pmu_test)+1):
        nodi_el = [nodi_el; jjj+find(pmu_test([jjj+1:len(pmu_test)]) ...
                                     == pmu_test(jjj))]
...
for i in range(1, len(pmutest)+1):
...
# posizionamento dei PMU, nodi ad interconnessione maggiore di 1
    ...
    while 1
      n_pmu = len(pmu_test)
      coll_idx = find(coll == j  and  pesi < 100)
for i in range(1, len(coll_idx)+1):
        pmu_idx = find(A(coll_idx(i),:))
        a = find(pesi(pmu_idx) >= 1)
        b = find(pesi(pmu_idx) > 99)
        if len(a) >= 1  and  len(b) < round(2*j/3)
...
for i in range(len(a), 1+1, -1):
        coll_idx = find(A(a(i),:))
        stoppa = 1
        if len(find(pesi(coll_idx) > 99)) < 2  and  len(coll_idx) > 1
...
for i in range(1, Bus.n+1):
  ...
  Matrix = cell(3,1)
  if len(index_pmu) > 1
    Matrix{3,1} = np.zeros((Bus.n+2,len(index_pmu)))
  else
    Matrix{3,1} = np.zeros((Bus.n,1))
...
for i in range(1, Bus.n+1):
  ...
  if type >= 4  and  len(index_pmu) > 1
for k in range(2, len(index_pmu)+1):
...
for i in range(1, Bus.n+1):
  ...
  if iscell(pseudoi)
    ...
    pseudi = pseudoi
  if iscell(I_idx)
    measui = len(I_idx{1}(:,1)) - pseudoi{1}
  else
    measui = len(I_idx(:,1)) - pseudoi
...
# Network and PMU statistics
  ...
  Matrix{2,1}(4,1) = len(pseudoi)
  if type < 4  or  len(index_pmu) == 1
...
for k in range(1, nconf+1):
      ...
      Matrix{3,1}(Bus.n+2,k) = len(I_idx{k}(:,1) - pseudoi{k,1})
...
# -----------------------------------------------------------
  ...
  fm_print(['PMU placement completed in ',durata],1)
