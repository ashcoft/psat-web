...
# Source: third-party/psat/filters\cloneblock.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
def cloneblock(varargin):
...
switch type
 ...
 case 'Shunt'
  ...
  elseif strmatch('Susceptance',a{2})
    template = 'Shunt1'
 case 'PV'
  if len(get_param(block,'MaskVarAliases')) == 7
    template = 'PV'
  else
    template = 'SC'
 case 'Line'
  if len(get_param(block,'MaskVarAliases')) == 7
    ...
    if not isempty(strfind(a,'shifter'))
      template = 'Transf3'
    elseif not isempty(strfind(a,'cable'))
      template = 'Line1'
    elseif not isempty(strfind(a,'tap ratio'))
      template = 'Transf2'
    else
      template = 'Line'
  elseif len(get_param(block,'MaskVarAliases')) == 8
    template = 'Transf4'
  elseif len(get_param(block,'MaskVarAliases')) == 6
    a = get_param(block,'MaskDescription')
    if not isempty(strfind(a,'auto'))
      template = 'Auto'
    else
      template = 'Transf1'
 otherwise
...
elseif strcmp(type,'Bus')
  values = get_param(block,'MaskValues')
  if len(values) == 4, updatebus = 1; end

ancestor = find_system('fm_lib','LookUnderMasks','functional','Name',template)

if isempty(clone)
  fm_print('* * Troubles in defining destination block')
elseif isempty(ancestor)
  fm_print('* * Troubles in finding library source (broken library link)')
else
  ...
  try
    ...
    if strcmp('Connected',a{end})  and  ...
          not strcmp('Connected',a_old{end})
      b = get_param(newblock,'MaskNames')
      set_param(newblock,b{end},'on')

  catch
    lasterr
    fm_print(['* * Troubles in replacing block <', ...
             clone,'> with <',ancestor,'>'])
