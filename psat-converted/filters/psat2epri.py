...
# Source: third-party/psat/filters\psat2epri.m  (upstream PSAT, GPL-2.0+)
...
if a == 2,
  ...
  fm_print(['File "',filename,'" not found or not an m-file'],2)
  check = 0
  return
...
fm_print(['Writing WSCC file "',newfile,'"...'])
...
# Bus type
  ...
  elseif not isempty(idxPV)
    if PV.con(idxPV,6) == 0  and  PV.con(idxPV,7) == 0
      count = fprintf(fid,'E    ')
    else
      count = fprintf(fid,'Q    ')
  elseif not isempty(idxPQ)
    if PQ.con(idxPQ,6) == 0  and  PQ.con(idxPQ,7) == 0
      count = fprintf(fid,'     ')
    else
      count = fprintf(fid,'V    ')
...
# Generator powers and limits
  if not isempty(idxPV)
    ...
    if PV.con(idxPV,6) != 0  or  PV.con(idxPV,7) != 0
      ...
      count = fprintf(fid,blanks(10))
  elseif not isempty(idxSW)
    PM = SW.con(idxSW,2)
    Pg = SW.con(idxSW,10)*SW.con(idxSW,2)
    count = fprintf(fid,[tr(PM,4),tr(Pg,5)],PM,Pg)
    if SW.con(idxSW,6) != 0  or  SW.con(idxSW,7) != 0
...
for i in range(1, Phs.n+1):
  ...
  count = fprintf(fid,tr(Bus.con(n,2),4),Bus.con(n,2))
  count = fprintf(fid,'%-5.2f%-5.2f', ...
		  Phs.con(i,13)*180/np.pi, ...
		  Phs.con(i,14)*180/np.pi)
