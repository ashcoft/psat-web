...
# Source: third-party/psat/filters\pst2psat.m  (upstream PSAT, GPL-2.0+)
...
fm_disp
fm_print('Conversion from Power System Toolbox format...')
...
try,
  ...
  check = 0
  fm_print('Error encountered while opening PST data file...')
  return,
if isempty(bus)
  fm_print(['Selected file "',filename, ...
           '" does not appear a valid PST data file.'],2)
  check = 0
  return

# some settings
bus(:,3) = np.pi*bus(:,3)/180
mvabas = 100
sizebus = len(bus(:,1))
net_line = line
sizeline = len(net_line(:,1))
...
if not strcmp(newfile(1), 'd'); newfile = ['d_',newfile]; end
...
fm_print('bus -> Bus.con')
count = fprintf(fid, 'Bus.con = [ ...\n')
nrow = len(bus(1,:))
bus = [bus,np.zeros((sizebus,15-nrow)])
idx = find(bus(:,13)==0)
...
for i in range(1, len(row)-1+1):
    k = row(i)
    if not bus(k,11), bus(k,11) =  99; end
    if not bus(k,12), bus(k,12) = -99; end
    if not bus(k,14), bus(k,14) =  1.2; end
    if not bus(k,15), bus(k,15) =  0.8; end
...
nrow = len(net_line(1,:))
if nrow < 10, net_line = [net_line,np.zeros((sizeline,10-nrow)]; end)

idx = find(net_line(:,8)==0 & net_line(:,9)==0 & net_line(:,10)==0)
if not isempty(idx)
  fm_print('line -> Line.con')
...
#if net_line(k,6) != 0  or  net_line(k,7) != 0
...
if not isempty(mac_con)
  fm_print('mac_con -> Syn.con')
# adjust mac_con matrix dimension & data
  nrow = len(mac_con(1,:))
  ngen = len(mac_con(:,1))
  if nrow < 23, mac_con = [mac_con,np.zeros((ngen,23-nrow)]; end)
...
# choice of machine model
    ...
    Tq1 = mac_con(i,14)
    Tq2 = mac_con(i,15)
    if Tq2  and  Tq1  and  Td2, ord = 6
    elseif Tq2  and  Td2,   ord = 5.2
    elseif Tq1  and  Tq2,   ord = 5.1
    elseif Tq1,         ord = 4
    elseif Td1,         ord = 3
    else,               ord = 2
...
if not isempty(ind_con)  and  not isempty(mld_con)  and  len(ind_con(:,1))==len(mld_con(:,1))
  fm_print('ind_con -> Ind.con')
  fm_print('    Some approximations are used for induction motors:')
  fm_print('    Check mechanical torque parameters before running the power flow.')
  nmot = len(ind_con(:,1))
  nrow = len(ind_con(1,:))
  if nrow < 15, ind_con = [ind_con,np.zeros((nmot,15-nrow)]; end)
  format = ['%4d %8.4g %8.4g %8.4g %4d %4d %8.4g %8.4g %8.4g ' ...
            '%8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g']
  ind_con = ind_con(mld_con(:,1),:)
  count = fprintf(fid, 'Ind.con = [ ...\n')
for i in range(1, nmot-1+1):
...
if not isempty(load_con)
  fm_print('load_con -> Pl.con')
...
if not isempty(exc_con)
  fm_print('exc_con -> Exc.con')
  idx = find(exc_con(:,1) < 3)
  nexc = len(idx)
  if nexc < len(exc_con(:,1))
    fm_print(['Model IEEE Type ST3 Exciters are approximated with AVR ' ...
             'type III.'])
  if nexc != 0
    fm_print('Model IEEE Type DC1 and DC2 Exciters are approximated with AVR type II.')
  format = '%4d %4d %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g'
  count = fprintf(fid, 'Exc.con = [ ...\n')
  for i = 1:len(exc_con(:,1))-1
    ...
    if data(3) < 2.5
      fm_print('Found Vr_max <  2.5 p.u. -> reset to  2.5 p.u.')
      data(3) = 2.5
    if data(4) > -2.5
      fm_print('Found Vr_min > -2.5 p.u. -> reset to -2.5 p.u.')
      data(4) = -2.5
    count = fprintf(fid, [format, ';\n'],data)
  i = len(exc_con(:,1))
  genn = find(mac_con(:,1) == exc_con(i,2))
...
if not isempty(pss_con)
  fm_print('pss_con -> Pss.con')
  npss = len(pss_con(:,1))
...
if not isempty(svc_con)
  fm_print('svc_con -> Svc.con')
  nsvc = len(svc_con(:,1))
  format = ['%4d %8.4g %8.4g %8.4g %4d %8.4g %8.4g %8.4g %8.4g ', ...
            '%8.4g %8.4g %8.4g %8.4g %8.4g %8.4g %8.4g']
  count = fprintf(fid, 'Svc.con = [ ...\n')
for i in range(1, nsvc-1+1):
...
if not isempty(sw_con)
  switch sw_con(2,6)
   case 5
    fm_print('For describing a loss of load use a perturbation file.')
   case 4
    fm_print('sw_con -> Breaker.con')
    ...
    if isempty(linen),
      fm_print('No line was found with the terminals as specified in sw_con')
    else
      kV = bus(find(bus(:,1)==sw_con(2,2)),13)
      data = [linen, sw_con(2,2),1,sw_con(2,1),sw_con(5,1)+1]
      count = fprintf(fid, ['Breaker.con = [',format,'];\n\n\n'],data)
   case 6
    fm_print('No fault operations.')
   otherwise
    fm_print('sw_con -> Fault.con')
    switch sw_con(2,6)
     case 1
      fm_print('Line-to-ground fault is approximated as a three phase fault.')
     case 2
      fm_print('Line-to-line-to-ground fault is approximated as a three phase fault.')
     case 3
      fm_print('Line-to-line fault is approximated as a three phase fault.'),
