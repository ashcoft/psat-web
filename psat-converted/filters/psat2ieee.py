...
# Source: third-party/psat/filters\psat2ieee.m  (upstream PSAT, GPL-2.0+)
...
if not strcmp(pathname,pwd)
  cd(pathname)

fm_disp
fm_print(['Opening PSAT file "',filename,'"...'])
...
if a == 2,
  ...
  fm_print(['File "',pathname,filesep,filename,'" not found or not an m-file'],2)
  check = 0
  return
...
if Bus.n > 9999  and  Bus.n <= 99999
  fm_print(['The number of buses is > 9999. Extended IEEE CDF will be ' ...
           'used.'])
if Bus.n > 99999
  fm_print(['Network dimension is too big for IEEE-CDF (max bus number ' ...
           '= 99999).'],2)
  check = 0
  return
...
fm_print(['Writing IEEE CDF file "',newfile,'"...'])
...
if s > 3  and  s < 10, card_I(44) = 'S'; else, card_I(44) = 'W'; end #  Season
...
for i in range(1, Bus.n+1):
  ...
  idxPQ = findbus(PQ,i)
  idxSW = findbus(SW,i)
  if not isempty(Shunt.con), idxSH = find(Shunt.bus == i); end
  ...
  a4 = sprintf('%6.4f%7.3f',[vbus(i),180*abus(i)/np.pi])

  if not isempty(idxPQ)
...
card_l(41:41+len(nline)-1) = nline
count = fprintf(fid,card_l)

# Line Data Cards
for i in range(1, Line.n+1):
  if Line.con(i,1) < 10000  and  Line.con(i,2) < 10000
    a1 = sprintf('%4d %4d  1 1  1 ',Line.con(i,[1 2]))
  elseif Line.con(i,1) < 10000  and  Line.con(i,2) >= 10000
  ...
  tap = Line.con(i,11)
  if tap == 1, tap = 0; end
  if Line.con(i,7)  or  tap  or  Line.con(i,12)
    if tap == 0, tap = 1; end
    a2 = '1'
