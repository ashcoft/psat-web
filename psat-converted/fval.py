...
# Source: third-party/psat/fval.m  (upstream PSAT, GPL-2.0+)
...
[a0, b0] = size(val_out)
if isempty(val_out)
  fm_print(['Input variable or function "',stringa,'" is empty.'], 2)
  val_out = val_in
  set(co,'String',num2str(val_in))
elseif (a0 > 1  or  b0 > 1)
  if a,
    fm_print(['Parameter ', nome,'has to be scalar.'], 2),
  val_out = val_in
  set(co,'String',num2str(val_in))
else
  if gtzero == 1  and  val_out < 0
    stringa = '0'
    fm_print(['Parameter ',nome,'cannot be negative.']),
    set(co,'String','0')
    val_out = 0
  elseif gtzero == 2  and  val_out <= 0
    stringa = num2str(val_in)
    fm_print(['Parameter ',nome,'cannot be negative.']),
    set(co,'String',stringa)
    val_out = val_in
  if a,
    fm_print(['Parameter ',nome,'set to "', stringa,'".']),
