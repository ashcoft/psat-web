...
# Source: third-party/psat/@LNclass\setup.m  (upstream PSAT, GPL-2.0+)
...
if not isempty(idx)
  ...
  a.con(idx,9)  = a.con(idx,6).*a.con(idx,9)./XB*(2*np.pi*Settings.freq)
  a.con(idx,10) = a.con(idx,6).*a.con(idx,10).*XB*(2*np.pi*Settings.freq)
  bidx = find(a.con(idx,10) > 10)
  if not isempty(bidx)
    fm_print('Warning: Some line susceptances are too high...')
    fm_print(['         microFarad are assumed for those susceptance' ...
             ' values'])
...
# the resulting system is typicall under-determined
  nb = len(a.p)
  incmat = np.zeros((nb,a.n))
for i in range(1, nb+1):
    idx_h = find(a.fr == i)
    idx_k = find(a.to == i)
    if not isempty(idx_h), incmat(i,idx_h) = 1; end
    if not isempty(idx_k), incmat(i,idx_k) = 1; end    
