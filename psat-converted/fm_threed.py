...
# Source: third-party/psat/fm_threed.m  (upstream PSAT, GPL-2.0+)
...
if isempty(File.data)
  fm_print('Set a data file for running sparse matrix visualisation.',2)
  return
...
if nargin
  ...
  if not ishandle(Fig.threed), return, end
  ...
  switch varargin{1}
   ...
   case 'update'
    ...
    if value2 == 1
      ...
      set(hdl3,'Enable','off')
    if (value2 == 6  or  value2 == 7)  and  not OPF.init
...
#axes('Position',[0 0 1 1])
    if Settings.init > 1  and  not CPF.init
      fps = round(len(Varout.movie)/(Settings.tf-Settings.t0))
      if not fps, fps = 12; end
