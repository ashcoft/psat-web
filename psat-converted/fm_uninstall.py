...
# Source: third-party/psat/fm_uninstall.m  (upstream PSAT, GPL-2.0+)
...
error_number = 0
lasterr('')
if not ishandle(Fig.update), Fig.update = fm_update; end
...
for i in range(1, nrows+1):
  if i == nrows, form = '%s'; end
  c1 = isempty(findstr(file{i},c_name))
  c2 = isempty(findstr(file{i},Comp.descr))  or  len(file{i}) < len(Comp.descr)
  if c1  and  c2, fprintf(fid,form,file{i}); end
fclose(fid)
...
for i in range(1, nrows+1):
  ...
  c3 = isempty(findstr(file{i},'end'))
  if c2, fprintf(fid,form,file{i}); end
  if not c3  and  not c2, n=n+1; end
  if n == 2, c2 = 1; end
fclose(fid)
