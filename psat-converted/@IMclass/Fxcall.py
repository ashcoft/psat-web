...
# Source: third-party/psat/@IMclass\Fxcall.m  (upstream PSAT, GPL-2.0+)
...
if im1
  ...
  DAE.Fx = DAE.Fx + sparse(is1,is1,(km(im1)-ks).*i2Hm(im1)-(not u(im1)),DAE.n,DAE.n)
...
if im3
  ...
  DAE.Fx = DAE.Fx + sparse(is3,is3, km(im3).*i2Hm(im3)-(not u(im3)),DAE.n,DAE.n)
...
if im5
  ...
  DAE.Fx = DAE.Fx + sparse(is5,is5, km(im5).*i2Hm(im5)-(not u(im5)),DAE.n,DAE.n)
