...
# Source: third-party/psat/fm_tstep.m  (upstream PSAT, GPL-2.0+)
...
# estimate the minimum time step
  ...
  elseif DAE.n == 1,
    ...
    try
      opts.disp = 0
      if DAE.n <= 6
        freq = max(abs(np.linalg.eig(full(DAE.Fx))))
      else
...
# set the minimum time step
  ...
  if Settings.fixt
    if Settings.tstep < 0
      fm_print('Error: fixed time step is negative or zero',2)
      fm_print('Automatic time step has been set',1)
      Settings.fixt = 0
    elseif Settings.tstep < Settings.deltatmin
      fm_print('Warning: fixed time step is less than estimated minimum time step',2)
