...
# Source: third-party/psat/fm_theme.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
def fm_theme(command):
...
case 'apply'
  ...
  if fid == -1,
    fm_print(['Can''t open file ',Path.themes,popupstr(Theme.hdl(2))],2),
    return,
  ...
  if not isunix  and  Settings.hostver >= 7  and  sum(Theme.color09) < 0.3
    Theme.color09 = [0 0 0]
...
case 'preview'
  ...
  if not isunix  and  Settings.hostver >= 7  and  sum(Tcolor09) < 0.3
    Tcolor09 = [0 0 0]
