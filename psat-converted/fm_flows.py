...
# Source: third-party/psat/fm_flows.m  (upstream PSAT, GPL-2.0+)
...
for i in range(1, nisland+1):
      buses{i} = find(Q(:,idx(i)))
      nbuses = len(buses{i})
      ngen = len(intersect(gen_bus,buses{i}))
      if strcmp(flag,'verbose')
        fm_print([' * * Sub-network ', num2str(i),' has ', ...
