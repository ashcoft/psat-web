...
# Source: third-party/psat/fm_axesdlg.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
def fm_axesdlg(varargin):
...
case 1
    ...
    if isempty(arg1)  or  ishandle(arg1)  or  isa(arg1(1),'scribehandle')
...
# Toggle the extra check box state
    ...
    elseif isequal(SibUd.Value,get(Sibling,'Value')),
        set(Sibling,'value',not get(Sibling,'Value'))
        value = get(Sibling,'Value')
    else
...
# Perform as before
    ...
    if not isempty(get(uic,'String')),
        iGroup = iGroup+2
        ActiveButton = localData.LimCheck(iGroup)
        value = not get(ActiveButton,'Value')
    else
        ActiveButton = uic
...
# Store the checkbox state,
        if not isempty(localData.Enable{iGroup}),
...
end #  if/else len(localData.axes...

# for the linear/log switches
disableGroup = localData.Disable{iGroup}
if not isempty(disableGroup)  and  enableflag,
...
# Check for similar titles
        ...
        if isempty(titleString{1})  or  ...
                (len(titleString) > 1  and  not isequal(titleString{:})),
...
# put down the row headings
        ...
        for iRow = 1:nRows,
            if not isempty(rowLabelStrings{iRow,1})
                headingPosition(2) = Y
                uicontrol(tProps,...
                    'FontWeight', 'bold',...
...
# Check for similar labels
            ...
            if isempty(labelString{1})  or  ...
                    (len(labelString)>1  and  not isequal(labelString{:})),
# use a cell array, so spaces aren't padded out.
                CommonFlag = 0
                labelString = {}
            else
...
# Check if all axes are manual or auto
            ...
            elseif all(AllMods)
                ...
                MultMods = 1
                checkProps.Value = len(find(AllMods))>=len(find(not AllMods))
...
# Check for common limits
            ...
            Umin = unique(lim(:,1))
            Umax = unique(lim(:,2))
            if len(Umin)>1, uminStr = ''
            else, uminStr = num2str(Umin)
            if len(Umax)>1, umaxStr = ''
...
# Check for common tick marks
                ...
                if (len(tick)>1  and  not isequal(tick{:}))  or  isempty(tick{1})  or  len(tick{1})<2
                    tickstep = []
                else
...
# Check if all axes grids are on
            ...
            elseif all(AllGrids)
                ...
                MultGrids = 1
                checkProps.Value = len(find(AllGrids))>=len(find(not AllGrids))
...
# finish opening axes dialog box
    ...
    case {'checkbox' 'radiobutton'}
        val = get(uic, 'Value')
        setflag = not strcmp(get(uic,'Enable'),'off')
        if setflag  and  isstruct(get(uic,'UserData'))
...
# title
        ...
        switch class(val)
        case 'char'
            if not isempty(val)  and  size(val,1)>1
...
# check for blank line at end of multiline title
        ...
        case 'cell'
            if len(val)>1
                nTitleLines = len(val)
                if isempty(val{nTitleLines})  or  val{nTitleLines}==32
                    val(nTitleLines) = []

        CommonTitle = get(findobj(f,'Tag','Title'),'UserData')
        if not (isempty(val)  and  len(ax)>1)  or  CommonTitle  or  len(ax)==1,
            t = get(ax,{'Title'})
            set([t{:}],'String',val)
            if not CommonTitle, #  They are common, now
                set(findobj(f,'Tag','Title'),'UserData',1)

        data = getappdata(f,'ScribeAxesDialogData')
        cols = 'XYZ'

for i in range(1, data.nCols+1):
            col = cols(i)
#label
            CommonLabel = get(findobj(f,'Tag',[col 'Label']),'UserData')
            val = getval(f,[col 'Label'])
            if not (isempty(val)  and  len(ax)>1)  or  CommonLabel  or  len(ax)==1,
                t = get(ax,{[col 'Label']})
                set([t{:}],'String',val)
                if not CommonLabel, #  They are common, now
...
# ranges checked on callbacks
                    if not (isnan(valMax)  or  isnan(valMin)),
                        iProp = iProp+1
            ...
            if setflag,
                ...
                if linearScale
                    if manual
                        ...
                        if not isempty(tickstep), #  Make sure something is there
...
# grid
            ...
            if not isempty(TickInd)  or  isnan(valMax)  or  isnan(valMin)
# Going to have to loop thru to set limits/ticks individually
for ct in range(1, len(ax)+1):
                    limits = get(ax(ct),[col 'Lim'])
                    axPropsCustom{1} = [col 'Lim']
                    if not isnan(valMax), limits(2) = valMax; end
                    if not isnan(valMin), limits(1) = valMin; end
                    axValsCustom{1} = limits

                    if not isempty(TickInd)
                        axPropsCustom{2} = [col 'Tick']
                        ticks = limits(1):tickstep:limits(2)
# ranges checked on callbacks
                        axValsCustom{2} = ticks
                    end #  if not isempty(TickInd)
