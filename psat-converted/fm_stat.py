...
# Source: third-party/psat/fm_stat.m  (upstream PSAT, GPL-2.0+)
...
for i in range(1, len(h)+1):
   ...
   case 'raddeg'
    ...
    switch get(hdl,'UserData')
     ...
     case 'rad'
      set(findobj(Fig.stat,'Tag','ListboxAng'),'String',setvar(abus*180/np.pi))
...
if isempty(File.data)
  fm_print('Set a data file for viewing static report.',2)
  return
...
if Bus.n > 150,
  fm_print(['Only the first 150 buses are reported in the Static', ...
      ' Report GUI'])
if DAE.n > 100,
  fm_print(['Only the first 100 state variables are reported in the Static', ...
      ' Report GUI'])
