...
# Source: third-party/psat/fm_del.m  (upstream PSAT, GPL-2.0+)
...
# initial variables
    ...
    Initl.name(ini_idx) = []
    Initl.n = Initl.n-len(ini_idx)

# GUI settings
    if isempty(Algeb.idx), set(hdl(13),'String',' ','Value',1)
    else, set(hdl(13),'String',Algeb.idx,'Value',len(Algeb.idx))
...
case 'ListboxServiceVar'
    ...
    Servc.eqidx(ser_idx) = []
    Servc.eq(ser_idx) = []
    Servc.neq = Servc.neq - len(ser_idx)
...
# GUI settings
    if not State.neq, set(hdl(10),'String',State.eq,'Value',1)
