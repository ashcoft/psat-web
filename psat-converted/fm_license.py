...
# Source: third-party/psat/fm_license.m  (upstream PSAT, GPL-2.0+)
...
if nargin
  ...
  if not exist('Fig'), Fig.license = -1, end
  if not ishandle(Fig.license)
  ...
  switch varargin{1}
   ...
   case 'keyesc'
    ...
    if (double(tasto) == 13  or  double(tasto) == 27), close(gcf); end

   case 'buttondown'

    pos = get(Fig.license,'CurrentPoint')
    if pos(1) > 0.05  and  pos(2) > 0.05  and  pos(1) < 0.95  and  pos(2) < 0.25
...
retidx = [0,retidx,len(helpstr)+1]
testo = cell(len(retidx)-1,1)
