...
# Source: third-party/psat/fm_laprint.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
import numpy as np

def fm_laprint(figno, filename, varargin):
...
vers=eval(vers(1:3))
if vers < 5.0
  fm_print('LaPrint Error: Matlab 5.0 or above is required.',2)
  return

# no output
if nargout
  fm_print('La Print Error: No output argument is required.',2)
  return
...
#%% Advanced Help %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
   case 'advancedhelp'
    print(' ')
    print('Advanced usage: fm_laprint(figno,filename,opt1,opt2,..)')
    print('where ')
    print('  figno      : integer, figure to be printed')
    print('  filename   : string, basename of files to be created')
    print('  opt1,..    : strings, describing optional inputs as follows')
    print('')
    print('   ''width=xx''      : xx is the width of the figure in the tex')
    print('                     file (in cm).')
    print('   ''factor=xx''     : xx is the factor by which the figure in ')
    print('                     the LaTeX document is smaller than the figure')
    print('                     in the postscript file.')
    print('                     A non-positive number for xx lets the factor')
    print('                     be computed such that the figure in the')
    print('                     postscript file has the same size as the')
    print('                     figure on screen.')
    print('   ''asonscreen''    : prints a graphics ''as on screen'',')
    print('                     retaining ticks, ticklabels and lims.')
    print('   ''verbose''       : verbose mode; asks before overwriting')
    print('                     files and issues some more messages.') 
    print('   ''keepticklabels'': keeps the tick labels within the eps-file')
    print('   ''mathticklabels'': tick labels are set in LaTeX math mode')
    print('   ''keepfontprops'' : tries to translate the MATLAB font') 
    print('                     properties (size, width, angle) into')
    print('                     similar LaTeX font properties.') 
    print('   ''noscalefonts''  : does not scale the fonts with the figure.') 
    print('   ''noextrapicture'': does not add extra picture environments.')
    print('   ''loose''         : uses ''-loose'' in the Matlab print command.')
    print('   ''nofigcopy''     : directly modifies the figure figno.')
    print('   ''nohead''        : does not place a commenting head in ')
    print('                     the tex-file. ')
    print('   ''caption=xx''    : adds \caption{xx} and \label{fig:filename}') 
    print('                     entries to the tex-file.')
    print('   ''comment=xx''    : places the comment xx into the header') 
    print('                     of the tex-file')
    print('   ''viewfile=xx''   : creates an additional file xx.tex')
    print('                     containing a LaTeX document which calls')
    print('                     the tex-file.')
    print(' ')
...
for hhh in range(1, len(psatfigs)+1):
    ...
    if not LAPRINTOPT.extrapicture
    ...
    eval(lapcmd)
   otherwise
    fm_print('LaPrint Error: unknown callback option!',2)
  return
...
if exist('LAPRINTHAN','var')
  if not isempty(LAPRINTHAN)
    directcall=0

if nargin==1
  if not isa(figno,'char')
    filename='unnamed'
  else  
    filename=figno
    figno=gcf
if not isa(figno,'double') 
  fm_print(['LaPrint Error: "',num2str(figno),'" is not a figure handle.'],2) 
  return
if not any(get(0,'children')==figno)
  fm_print(['LaPrint Error: "',num2str(figno),'" is not a figure handle.'],2) 
  return
if not isa(filename,'char')
  filename
  fm_print(['La Print Error: file name is not valid.'],2) 
  return
...
if nargin>2  
for i in range(1, nargin-2+1):
    if not isa(varargin{i},'char')
      fm_print('LaPrint Error: Options must be character arrays.',2)
      return
    oriopt=varargin{i}(:)';
	...
	captiontext=oriopt(eqpos+1:len(oriopt))
    elseif strcmp(opt(1:8),'comment=')
      eqpos=findstr(oriopt,'=')
      furtheroptions=[furtheroptions ' / ' oriopt ]
      commenttext=oriopt(eqpos(1)+1:len(oriopt))
    elseif strcmp(opt(1:9),'viewfile=')
      viewfile=1
      eqpos=findstr(oriopt,'=')
      furtheroptions=[furtheroptions ' / ' oriopt ]
      viewfilename=oriopt(eqpos(1)+1:len(oriopt))
    elseif strcmp(opt(1:6),'width=')
      eval([ opt ';' ])
    elseif strcmp(opt(1:7),'factor=')
      eval([ opt ';' ])
    else
      fm_print(['LaPrint Error: Option ' varargin{i} ' not recognized.'],2)
      return
...
if verbose, 
  fm_print(['This is LaPrint, version ',laprintident,'.'])
...
if not strcmp(texdirname,epsdirname)
  fm_print(['LaPrint Warning: eps-file and tex-file are placed in ' ...
...
if viewfile
  ...
  if strcmp(texfullnameext,viewfullnameext)
    fm_print(['LaPrint Error: The tex- and view-file coincide. Use ' ...
             'different names.'],2)
    return
  if not strcmp(texdirname,viewdirname)
    fm_print(['LaPrint Warning: eps-file and view-file are placed '...
...
# some warnings
if directcall
  if (epswidth<13)  or  (epsheight<13*0.75)
    print('warning: The size of the eps-figure is quite small.')
    print('         The text objects might not be properly set.')
    print('         Reducing ''factor'' might help.')
  if latexwidth/epswidth<0.5
    print([ 'warning: The size of the eps-figure is large compared ' ...
           'to the latex figure.' ])
    print('         The text size might be too small.')
    print('         Increasing ''factor'' might help.')
  if (orip(3)-epswidth)/orip(3) > 0.1
    print(['warning: The size of the eps-figure is much smaller '...
	  'than the original'])
    print('         figure on screen. Matlab might save different ticks and')
    print([ '         ticklabels than in the original figure. See option ' ...
           '''asonsceen''. ' ])
...
for i in range(1, nt+1):
  if isa(oldstr{i},'cell')
    if len(oldstr{i})>1
      print('LaPrint warning: Annotation in form of a cell is currently')
      print('         not supported. Ignoring all but first component.')
      iswarning=1
# To do: place a parbox here. 
    oldstr{i}=oldstr{i}{1}
  if size(oldstr{i},1)>1
    print([ 'LaPrint warning: Annotation in form of string matrices ' ...
           'is currently not supported.' ])
    print('         Ignoring all but first row.')
    iswarning=1
# To do: place a parbox here. 
    oldstr{i}=oldstr{i}(1,:)
  if len(oldstr{i})
...
for i in range(1, nt+1):
    ...
    elseif strcmp(fontweight{i},'bold')
      ...
      print([ ' LaPrint warning: unknown fontweight:' fontweight{i} ])
      iswarning=1
      fontweightcmd{i}=[ '\\fontseries{m}\\mathversion{normal}' ]
...
for i in range(1, nt+1):
    ...
    elseif strcmp(fontangle{i},'oblique')
      ...
      print([ ' LaPrint warning: unknown fontangle:' fontangle{i} ])
      iswarning=1
      fontanglecmd{i}=[ '\\fontshape{n}' ]
...
for i in range(1, na+1):
    ...
    if issuewarning
      print('LaPrint warning: There seems to be a 3D plot. The LaTeX labels are')
      print('         possibly incorrect. The option  ''keepticklabels'' might')
      print('         help. The option ''nofigcopy'' might be wise, too.')

# try to figure out if we linear scale with extra factor 
# and determine powers of 10
  powers=np.nan*np.zeros((na,3);  #  matrix with powers of 10)
  for i=1:na                    #  all axes
    allxyz={ 'x', 'y', 'z' }
    for ixyz=1:3                #  x,y,z
      xyz=allxyz{ixyz}
	...
	if nticks==0,
	  ...
	  if len(label)==0, 
	    powers(i,ixyz)=0
	    break
	  if ( label==0 )  and  ( abs(tick(k))>1e-10 )
	    powers(i,ixyz)=0
	    break
	  if label!=0    
...
# replace all ticklabels and bookkeep
  nxlabel=np.zeros((1,na))
  nylabel=np.zeros((1,na))
  nzlabel=np.zeros((1,na))
  allxyz={ 'x', 'y', 'z' }
for ixyz in range(1, 3+1):
    xyz=allxyz{ixyz}
    k=1
    basestr=[ xyz '00' ]
...
# font size
    afsize=np.zeros((na,1))
for i in range(1, na+1):
      afsize(i)=get(hax(i),'fontsize')
    if (any(afsize != afsize(1) ))
      print('LaPrint warning: Different font sizes for axes not supported.')
      print([ '         All axses will have font size ' ...
	     num2str(afsize(1)) '.' ] )
      iswarning=1
...
for i in range(1, na+1):
    ...
    elseif strcmp(afweight{1},'bold')
      ...
      print([ ' LaPrint warning: unknown fontweight:' afweight{1} ])
      iswarning=1
      afweightcmd=[ '\\fontseries{m}\\mathversion{normal}' ]
for i in range(1, na+1):
      if not strcmp(afweight{i},afweight{1})
        print(' LaPrint warning: Different font weights for axes not')
        print([ '      supported. All axes will have font weight ' afweightcmd])
        iswarning=1
...
for i in range(1, na+1):
    ...
    elseif strcmp(afangle{1},'oblique')
      ...
      print([ ' LaPrint warning: unknown fontangle:' afangle{1} ])
      iswarning=1
      afanglecmd=[ '\\fontshape{n}' ]
for i in range(1, na+1):
      if not strcmp(afangle{i},afangle{1})
        print('LaPrint warning: Different font angles for axes not supported.')
        print([ '         All axes will have font angle ' afanglecmd ] )
        iswarning=1
...
if extrapicture
  unitlength=np.zeros((na,1))
  ybound=np.zeros((na,1))
for i in range(1, na+1):
    if not is3d(hax(i))
      xlim=get(hax(i),'xlim')
      ylim=get(hax(i),'ylim')
      ...
      unitlen(i)=pos(3)*epswidth
      ybound(i)=(pos(4)*epsheight)/(pos(3)*epswidth)
    else
      if directcall
	print('LaPrint warning: Option ''extrapicture'' for 3D axes not supported.')
...
#
if verbose
  print([ 'writing to: '' ' texfullnameext ' ''' ])
...
numbertext=0
for i in range(1, nt+1):
  numbertext=numbertext+len(newstr{i})
if numbertext>0,
  fprintf(fid,'%%\n')
...
if not keepticklabels
  ...
  nzlabel=np.zeros((1,na))
for i in range(1, na+1):
    nxlabel(i)=len(newxtl{i})
    nylabel(i)=len(newytl{i})
    nzlabel(i)=len(newztl{i})
...
if viewfile
  if verbose
    print([ 'writing to: '' ' viewfullnameext ' ''' ])
  ...
  if not strcmp(epsdirname,viewdirname)
...
if not directcall  and  iswarning
...
if LAPRINTOPT.factor<0.5
  ...
  showtext({txt2},'add')
if ((figpos(3)-epswidth)/figpos(3)>0.1)  and  not LAPRINTOPT.asonscreen
...
if verbose
  ...
  while quest
    ...
    if strcmp(yn,'y') 
      ...
      basenameext = [ basename '.' extension ]
      quest = (exist(fullnameext)==2)
if ( exist(dirname)!=7  and  not strcmp(dirname,[ '.' filesep ]) ...
      and  not strcmp(dirname,filesep) )
  fm_print(['LaPrint Error: Directory ',dirname,' does not exist.'],2)
  return
...
if nslash
  ...
  dirname = pwd
  nn=len(dirname)
  if not strcmp(dirname(nn),filesep)
