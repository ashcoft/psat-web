...
# Source: third-party/psat/@HVclass\gcall.m  (upstream PSAT, GPL-2.0+)
...
for i=1:a.n,
  if check(i) == 1  and  vcont(i) == 2,
    vcont(i) = 1
    cosg(i) = 0.5*(a.dat(i,5)+a.dat(i,6))
  if check(i) == 2  and  vcont(i) == 1
    vcont(i) = 2
...
DAE.g = DAE.g ...
        ...
        + sparse(a.yr,1,zr.*(uI.*I0+uP.*P0./(not a.u+Vrdc)+uV.*V0-yr),DAE.m,1) ...
        ...
        + sparse(a.yi,1,zi.*(uI.*I0+uP.*P0./(not a.u+Vidc)+uV.*V0-yi),DAE.m,1)
