...
# Source: third-party/psat/fm_gams.m  (upstream PSAT, GPL-2.0+)
...
[u,w] = system('gams')
if u
  fm_print('GAMS is not properly installed on your system.',2)
  return

if not autorun('PSAT-GAMS Interface',0)
  return

if DAE.n
  fm_print(['Dynamic data are not supported within the PSAT-GAMS interface.'],2)
  return

if not Supply.n,
  fm_print(['Supply data have to be specified before in order to ', ...
	   'run PSAT-GAMS interface'],2)
  return

if not Demand.n,
  if GAMS.basepg  and  not clpsat.init
    Settings.ok = 0
    uiwait(fm_choice(['Exclude (recommended) base generator powers?']))
    GAMS.basepg = not Settings.ok
  noDem = 1
...
type = GAMS.type
omega = GAMS.omega
if GAMS.method == 6  and  GAMS.type != 1
  fm_print(['WARNING: Continuation OPF can be run only with Single' ...
	   ' Period Auctions.'])
  fm_print('Voltage Stability Constrained OPF will be solved.')
  method = 4
if GAMS.method == 6  and  GAMS.flow != 1
  fm_print(['WARNING: Continuation OPF can be run only with Current ' ...
           'Limits.'])
  fm_print('Current limits in transmission lines will be used.')
  GAMS.flow = 1
if GAMS.type == 3  and  GAMS.method != 4
  fm_print(['WARNING: Pareto Set Single Period Auction can be run ' ...
           'only for VSC-OPF.'])
  fm_print( '         Single Period Auction will be solved.')
  fm_disp
  type = 1
if GAMS.type == 3  and  len(GAMS.omega) == 1
  fm_print(['WARNING: Weighting factor is scalar. ', ...
           'Single Period Auction will be solved.'])
  fm_disp
  type = 1
if GAMS.type == 1  and  len(GAMS.omega) > 1
  fm_print(['WARNING: Weighting factor is a vector. ', ...
           'First omega entry will be used.'])
  fm_disp
  omega = omega(1)
if not rem(GAMS.type,2)  and  not Rmpg.n
  type = 1
  fm_print(['WARNING: No Ramping data were found. ', ...
           'Single Period Auction will be solved.'])
  fm_disp
if GAMS.type == 2  and  Rmpg.n  and  not Ypdp.n
  type = 4
  fm_print(['WARNING: No Power Demand Profile was found. Single ' ...
...
switch method
 case 1, fm_print(' Simple Auction')
 case 2, fm_print(' Market Clearing Mechanism')
 case 3, fm_print(' Standard OPF')
 case 4, fm_print(' Voltage Stability Constrained OPF')
 case 5, fm_print(' Maximum Loading Condition')
 case 6, fm_print(' Continuation OPF')
 case 7, fm_print(' Congestion Management')

switch type
 case 1, fm_print(' Single-Period Auction')
 case 2, fm_print(' Multi-Period Auction')
 case 3, fm_print(' Pareto Set Single-Period Auction')
 case 4, fm_print(' Single-Period Auction with Unit Commitment')

if (GAMS.flatstart  or  isempty(Snapshot))  and  GAMS.method > 2
  DAE.y(Bus.a) = getnp.zeros((Bus))
...
if clpsat.init  or  ispc, cd(Path.psat), end
...
# ------------------------------------------------------------------
  ...
  elseif not rem(type,2) #  Single/Multi Period Auction with UC
    [Ps,Pd,MCP,modelstat,solvestat] = psatgams(file,nBus,nLine,nPs,nPd,nSW,nPV, ...
                       nBusref,nH,control,flow,S,D,X,Ch)
    numh = size(MCP,1)
    a = np.zeros((numh,Bus.n))
    V = np.zeros((numh,Bus.n))
    Qg = np.zeros((numh,Bus.n))
    Pij = np.zeros((numh,Line.n))
    Qij = np.zeros((numh,Line.n))
...
#lambda.val(1) = lambda_values(i);
  ...
  idx_gen = np.zeros((Supply.n,1))
  Psc_idx = Ps_idx
  Psm_idx = np.zeros((Bus.n,Supply.n))
  ...
  while 1
    ...
    if iteration > 10
      fm_print('* * * Maximum number of iteration with no convergence!')
      break
    idx = psupper(Supply,(1+lambdac+kg)*Ps)
    if sum(idx_gen(idx)) == len(idx)
      fm_print(['* * * iter = ',num2str(iteration), ...
               ', #viol., ',num2str(len(find(idx_gen))), ...
...
# loop until there are no violations of power supply limits
      idx_gen(idx) = 1
      fm_print(['* * * iter = ',num2str(iteration),', #viol. = ', ...
               num2str(len(idx)),', lambda = ', ...
               num2str(lambdac),' kg = ', num2str(kg)])
      drawnow
      Psc_idx = psidx(Supply,not idx_gen)
...
# ------------------------------------------------------------------
  ...
  elseif not rem(type,2) #  Single/Multi Period Auction with UC
    ...
    NCP = np.zeros((len(lambdac),Bus.n))
for i in range(1, len(lambdac)+1):
      NCPi = compNCP(V(i,:)',a(i,:)',mV(i,:)',mFij(i,:)',mFji(i,:)');
      NCP(i,:) = NCPi';
...
for i in range(1, len(omega)+1):
      fm_print(sprintf(' VSC-OPF #%d, %3.1f%% - omega: %5.4f', ...
		      i,100*i/len(omega),omega(i)))
...
# ------------------------------------------------------
      i = i + 1
      fm_print(sprintf('Continuation OPF #%d, lambda_c = %5.4f', ...
		      i,lmin))
...
# check consistency of the solution (LMP > 0)
      if modelstat > 3 # min(abs(roi)) < 1e-5
        ...
        fm_print('Unfeasible OPF solution. Discarding last solution.')
        Lambda(end) = []
        Ps(end,:) = []
        Pd(end,:) = []
        V(end,:) = []
...
# ------------------------------------------------------
      ...
      while 1
        ...
        if abs(ml) > 1e-5
          ...
          delta_max = np.linalg.norm([deltaPs; deltaPd])
          if delta_max == 0, delta_max = 1; end
          deltaPd = deltaPd/delta_max
          deltaPs = deltaPs/delta_max
        else
          deltaPd = np.zeros((Demand.n,1))
          deltaPs = np.zeros((Supply.n,1))
#ml
#mPceq
...
# reset admittance line
        ...
        if isempty(CPF.lambda)
          fm_print([' * CPF solution: <empty>'])
        elseif isnan(CPF.lambda)
          fm_print([' * CPF solution: <NaN>'])
        else
          fm_print([' * CPF solution: ',num2str(CPF.lambda-1)])
        ...
        if CPF.lambda < lambdaci  and  abs(ml) <= 1e-5
          ml = 0
          CPF.lambda = lmin+1e-5
        if CPF.lambda < lmin  and  abs(ml) > 1e-5
          fm_print([' * Decrease Delta Ps and Delta Pd'])
          delta = 0.5*delta
          if delta < 5e-8
            fm_print([' * CPF method cannot find a higher lambda'])
            stop_opf = 1
            break
          repeat_cpf = 1
        else
...
# maximum lambda increment
        if (CPF.lambda - lmin) > 0.025 #   and  (abs(ml) > 1e-5  or  CPF.lambda > 0.6)
          fm_print(['lambda critical = ',num2str(CPF.lambda)])
          fm_print(['Limit lambda increment to threshold (0.025)'])
          CPF.lambda = lmin + 0.025

# stopping criterion
# ------------------------------------------------------
        if last_point
          fm_print('Reached maximum lambda.')
          if CPF.lambda > lmin
            fm_print('Desired maximum lambda is not critical.')
          stop_opf = 1
          break
        if i >= CPF.nump
          fm_print('Reached maximum # of continuation steps.')
          stop_opf = 1
          break
        if CPF.lambda >= GAMS.lmax
          ...
          last_point = 1
        if CPF.lambda == 0
          fm_print('Base case solution is likely unfeasible.')
          stop_opf = 1
          break
        if abs(lmin-CPF.lambda) < 1e-5
#fm_disp([' or lambda(i+1) - lambda(i) or  = ', ...
#         num2str(abs(lmin-CPF.lambda))])
          fm_print('Lambda increment is lower than the desired tolerance.')
          stop_opf = 1
          break
        elseif not repeat_cpf
          if abs(ml) < 1e-5
...
#Pd = DPd;
    ...
    fm_print('Continuation OPF not implemented yet...')
    cd(currentpath)
    return
...
if type == 2  or  type == 3
  ...
  if GAMS.method == 4  or  GAMS.method == 6
    MLC = TTL.*(1+lambdac)
  ...
  if GAMS.method > 2  and  GAMS.method != 5
  ...
  Varout.idx = [1:len(Varout.vars(1,:))]

  fm_print(' ---------------------------------------------------------------')
  fm_print([' Check file ',Path.psat,'fm_gams.lst for GAMS report.'])
  if strcmp(control,'6')
    fm_print([' PSAT-GAMS Optimization Routine completed in ', ...
             num2str(etime(clock,initial_time)),' s'])
  else
    fm_print([' PSAT-GAMS Optimization Routine completed in ',num2str(toc),' s'])
...
if (Settings.showlf  or  GAMS.show)  and  clpsat.showopf

  fm_disp
  fm_print(' Power Supplies')
  fm_print(' ---------------------------------------------------------------')
  [Psmax,Psmin] = plim(Supply)
  if GAMS.method == 7
    fm_print({'Bus','Ps','Ps max','Ps min','dPs_up','dPs_dw'})
    fm_print({'<i>','[MW]','[MW]','[MW]','[MW]','[MW]'})
    fm_print([getidx(Bus,Supply.bus),Ps*MVA,Psmax*MVA,Psmin*MVA,dPSup*MVA,dPSdw*MVA])
  else
    fm_print({'Bus','Ps','Ps max','Ps min'})
    fm_print({'<i>','[MW]','[MW]','[MW]'})
    fm_print([getidx(Bus,Supply.bus),Ps*MVA,Psmax*MVA,Psmin*MVA])
  fm_disp
  fm_print(' Power Demands')
  fm_print(' ---------------------------------------------------------------')
  [Pdmax,Pdmin] = plim(Demand)
  if GAMS.method == 7
    fm_print({'Bus','Pd','Pd max','Pd min','dPd_up','dPd_dw'})
    fm_print({'<i>','[MW]','[MW]','[MW]'})
    fm_print([getidx(Bus,Demand.bus),Pd*MVA,Pdmax*MVA,Pdmin*MVA,dPDup*MVA,dPDdw*MVA])
  else
    fm_print({'Bus','Pd','Pd max','Pd min'})
    fm_print({'<i>','[MW]','[MW]','[MW]'})
    fm_print([getidx(Bus,Demand.bus),Pd*MVA,Pdmax*MVA,Pdmin*MVA])
  fm_disp
  fm_print(' Generator Reactive Powers')
  fm_print(' ---------------------------------------------------------------')
  if GAMS.method == 4  or  GAMS.method == 6
    fm_print({'Bus','Qg','Qgc','Qg max','Qg min'})
    fm_print({'<i>','[MVar]','[MVar]','[MVar]','[MVar]'})
    fm_print([getidx(Bus,iBQg),Qg*MVA,Qgc(iBQg)*MVA,Qgmax*MVA,Qgmin*MVA])
  else
    fm_print({'Bus','Qg','Qg max','Qg min'})
    fm_print({'<i>','[MVar]','[MVar]','[MVar]'})
    fm_print([getidx(Bus,iBQg),Qg*MVA,Qgmax*MVA,Qgmin*MVA])
  fm_disp
  fm_print(' Power Flow Solution')
  fm_print([' ----------------------------------------------------' ...
           '-----------'])
  fm_print({'Bus','V','theta','PG','PL','QG','QL'})
  fm_print({'<i>','[p.u.]','[rad]','[MW]','[MW]','[MVar]','[MVar]'})
  fm_print([getidx(Bus,0),DAE.y(Bus.v),DAE.y(Bus.a),PG,PL,QG,QL])

  fm_disp
  fm_print(' Prices and Pays')
  fm_print([' ----------------------------------------------------' ...
           '-----------'])
  if GAMS.method == 3  or  GAMS.method == 4  or  GAMS.method == 6
    fm_print({'Bus','LMP','NCP','Pay S','Pay D'})
    fm_print({'<i>','[$/MWh]','[$/MWh]','[$/h]','[$/h]'})
    fm_print([getidx(Bus,0),ro(Bus.a), NCP, PayS, PayD])
  else
    fm_print({'Bus','LMP','Pay S','Pay D'})
    fm_print({'<i>','[$/MWh]','[$/h]','[$/h]'})
    fm_print([getidx(Bus,0),ro(Bus.a), PayS, PayD])
  if GAMS.method == 4  or  GAMS.method == 6
    fm_disp
    fm_print(' "Critical" Power Flow Solution')
    fm_print(' ---------------------------------------------------------------')
    fm_print({'Bus','Vc','thetac','PGc','PLc','QGc','QLc'})
    fm_print({'<i>','[p.u.]','[rad]','[MW]','[MW]','[MVar]', ...
             '[MVar]'})
    PG = (1+lambdac+kg)*PG
    PL = (1+lambdac)*PL
    QL = (1+lambdac)*QL
    fm_print([getidx(Bus,0),Vc,ac,PG,PL,Qgc*MVA,QL])
  fm_disp
  if GAMS.flow
    fm_print(' Flows on Transmission Lines')
    fm_print(' ---------------------------------------------------------------')
    switch GAMS.flow
     case 1,
      fm_print({'From Bus','To Bus','Iij','Iijmax', ...
               'Iij margin','Iji','Ijimax','Iji margin'},1)
     case 2,
      fm_print({'From Bus','To Bus','Pij','Pijmax', ...
               'Pij margin','Pji','Pjimax','Pji margin'},1)
     case 3,
      fm_print({'From Bus','To Bus','Sij','Sijmax', ...
               'Sij margin','Sji','Sjimax','Sji margin'},1)
    fm_print({'<i>','<j>','[p.u.]','[p.u.]', ...
	     '[p.u.]','[p.u.]','[p.u.]','[p.u.]'})
    fm_print([Line.fr, Line.to,Pij, ...
             L.val(:,5),abs((-abs(Pij)+L.val(:,5))), ...
             Pji,L.val(:,5),abs((-abs(Pji)+L.val(:,5)))])
    fm_disp
  else
    fm_print('Flow limits are disabled.')
  if GAMS.method == 4  or  GAMS.method == 6
    fm_print(' Flows on Transmission Lines of the "Critical" System')
    fm_print(' ---------------------------------------------------------------')
    switch GAMS.flow
     case 1,
      fm_print({'From Bus','To Bus','Iijc','Iijcmax', ...
               'Iijc margin','Ijic','Ijicmax','Ijic margin'})
     case 2,
      fm_print({'From Bus','To Bus','Pijc','Pijcmax', ...
               'Pijc margin','Pjic','Pjicmax','Pjic margin'})
     case 3,
      fm_print({'From Bus','To Bus','Sijc','Sijcmax', ...
               'Sijc margin','Sjic','Sjicmax',['Sjic margin']})
    fm_print({'<i>','<j>','[p.u.]','[p.u.]', ...
	     '[p.u.]','[p.u.]','[p.u.]','[p.u.]'})
    if GAMS.flow
      fm_print([Line.fr, Line.to,Pijc, ...
  ...
  fm_print(' ---------------------------------------------------------------')
  if GAMS.method >= 4,
    fm_print([' omega = ',num2str(omega(1))])
    fm_print([' lambda_c = ',num2str(lambdac),' [p.u.]'])
    fm_print([' kg = ',num2str(kg),' [p.u.]'])
  if GAMS.method == 1
    fm_print([' Market Clearing Price = ',num2str(MCP),' [$/MWh]'])

  total_loss = 1e-5*round(sum(Line.p)*1e5)*MVA
  bid_loss = 1e-5*round((sum(Line.p)-Snapshot(1).Ploss)*1e5)*MVA

  fm_print([' Total Losses = ',num2str(total_loss),' [MW]'])
  fm_print([' Bid Losses = ',num2str(bid_loss),' [MW]'])
  fm_print([' Total demand = ',num2str(sum(Pd)*MVA),' [MW]'])
  fm_print([' Total Transaction Level = ', ...
           fvar(sum(Pd)*MVA+TPQ,8),' [MW]'])
  if GAMS.method == 4  or  GAMS.method == 6
    fm_print([' Maximum Loading Condition = ', ...
             fvar((1+lambdac)*(sum(Pd)*MVA+TPQ),8),' [MW]'])
    fm_print([' Available Loading Capability = ', ...
             fvar(lambdac*(sum(Pd)*MVA+TPQ),8),' [MW]'])
  if GAMS.method == 5
    fm_print([' Maximum Loading Condition = ', ...
             fvar(lambdac*(sum(Pd)*MVA+TPQ),8),' [MW]'])
  fm_print([' IMO Pay = ',num2str(ISOPay),' [$/h]'])
  fm_disp
...
dH_dtV = Jij'*mFij + Jji'*mFji + [getnp.zeros((Bus);mV])
dH_dtV(SW.refbus,:) = 0
NCP = DAE.Gy'\dH_dtV;
NCP = NCP(Bus.a)
...
fm_print([' GAMS routine completed in ',num2str(etime(clock,t0)),' s'])
if status
  fm_print(result)
  return
...
psatsol
if nout < nargout
for i in range(nout+1, nargout+1):
    varargout{i} = []
...
switch status
 case 0, fm_print(' GAMS model status: not available')
 case 1, fm_print(' GAMS model status: optimal')
 case 2, fm_print(' GAMS model status: locally optimal')
 case 3, fm_print(' GAMS model status: unbounded')
 case 4, fm_print(' GAMS model status: infeasible')
 case 5, fm_print(' GAMS model status: locally infeasible')
 case 6, fm_print(' GAMS model status: intermediate infeasible')
 case 7, fm_print(' GAMS model status: intermediate non-optimal')
 case 8, fm_print(' GAMS model status: integer solution')
 case 9, fm_print(' GAMS model status: intermediate non-integer')
 case 10, fm_print(' GAMS model status: integer infeasible')
 case 11, fm_print(' GAMS model status: ???')
 case 12, fm_print(' GAMS model status: error unknown')
 case 13, fm_print(' GAMS model status: error no solution')
 otherwise, fm_print(' GAMS model status: unknown model status')
...
switch status
 case 0, fm_print(' GAMS solver status: not available')
 case 1, fm_print(' GAMS solver status: normal completion')
 case 2, fm_print(' GAMS solver status: iteration interrupt')
 case 3, fm_print(' GAMS solver status: resource interrupt')
 case 4, fm_print(' GAMS solver status: terminated by solver')
 case 5, fm_print(' GAMS solver status: evaluation error limit')
 case 6, fm_print(' GAMS solver status: unknown')
 case 7, fm_print(' GAMS solver status: ???')
 case 8, fm_print(' GAMS solver status: error preprocessor error')
 case 9, fm_print(' GAMS solver status: error setup failure')
 case 10, fm_print(' GAMS solver status: error solver failure')
 case 11, fm_print(' GAMS solver status: error internal solver error')
 case 12, fm_print(' GAMS solver status: error post-processor error')
 case 13, fm_print(' GAMS solver status: error system failure')
 otherwise, fm_print(' GAMS solver status: unknown solver status')
