...
# Source: third-party/psat/psat.m  (upstream PSAT, GPL-2.0+)
...
a = version
a = str2num(a(1:3))
if not exist('OCTAVE_VERSION')
  if a < 5.3
    print('PSAT needs a Matlab version >= 5.3')
    return
  elseif a < 7.0
    print(['For Matlab versions < 7.0, PSAT runs only ' ...
...
print(' ')
print('                        < P S A T >')
print('          Copyright (C) 2002-2016 Federico Milano')
print([blanks(30-floor(5+len(psatver)/2)),'Version ',psatver])
print([blanks(30-ceil(len(psatdate)/2)),psatdate])
print('  ')
print('PSAT comes with ABSOLUTELY NO WARRANTY; type ''gnuwarranty''')
print('for details. This is free software, and you are welcome to')
print(['redistribute it under certain conditions; type' ...
      ' ''gnulicense'''])
print('for details.')
print('  ')
if exist('OCTAVE_VERSION')
  print(['Host:         Octave ',version])
else
  print(['Host:         Matlab ',version])
print(['Session:      ', datestr(now,0)])
if command_line_psat
  print('Usage:        Command Line')
else
  print('Usage:        Graphical User Interface')
print(['Path:         ',pwd])
print(' ')
...
# check for Octave
if exist('OCTAVE_VERSION')  and  isempty(findstr(path,[psatdir,filesep,'@BUclass']))
...
if exist(Path.build) != 7
  print('No "build" folder found in the current PSAT path.')
  failed = not mkdir(Path.psat,'build')
  if failed,
    print('The folder "build" could not be created')
if exist(Path.images) != 7
  print('No "images" folder found in the current PSAT path.')
  failed = 1
if exist(Path.themes) != 7
  print('No "themes" folder found in the current PSAT path.')
  failed = not mkdir(Path.psat,'themes')
  if failed,
    print('The folder "themes" could not be created')
if exist(Path.filters) != 7
  print('No "filters" folder found in the current PSAT path.')
  failed = 1
if failed
  print('The PSAT folder seems to be uncomplete and needs to be installed again.')
  print('PSAT initialization failed.')
  return

if not command_line_psat
...
try
  ...
  Theme = struct('color01',[0.800 0.800 0.800], ...
                 ...
                 'font01','Courier','hdl',np.zeros((18,1))
...
if not isfield(Settings,'local')
  Settings.local = 1

if isunix  and  strcmp(Settings.export,'xls')
  Settings.export = 'txt'
...
if Settings.octave
  ...
  elseif strncmp(computer,'MAC',3)  and  usejava('jvm')
...
if fid == -1,
  print('#Error: File "namevarx.ini" cannot be open.')
  failed = 1
else
  nname = 0
  while 1
    ...
    try
      ...
      if Settings.octave  and  strcmp(Varname.compx{nname+1,1}(end),'\r')
...
if fid == -1,
  print('#Error: File "namevary.ini" cannot be open.')
  failed = 1
else
  nname = 0
  while 1
    ...
    try
      ...
      if Settings.octave  and  strcmp(Varname.compy{nname+1,1}(end),'\r')
...
if fid == -1,
  print('#Error: File "history.ini" cannot be open.')
  failed = 1
else
  while 1
    sline = fgetl(fid)
...
if fid == -1,
  print('#Error: File "comp.ini" cannot be open.')
  failed = 1
else
  ncomp=0
  while 1
...
if Settings.matlab  and  Settings.hostver >= 7
  if not exist('load_system')
    if exist('simulink') != 5
      fm_print('* * Simulink cannot be found on your system.')
      fm_print('* * The PSAT-Simulink will not be available.')
    else
      fm_print('* * Problems in loading the PSAT Simulink Library.')
  else
    load_system('fm_lib')

if failed,
  print(' '), print('PSAT is not properly initialized.')
else,
  if not command_line_psat, fm_main, end

if not command_line_psat, close(hdl), end
