...
# Source: third-party/psat/@DFclass\setx0.m  (upstream PSAT, GPL-2.0+)
...
# etaGB*4*R*pi*f/p
a.dat(:,4) = 4*np.pi*Settings.freq*a.con(:,16)./a.con(:,17).*a.con(:,19)
# A
a.dat(:,5) = np.pi*a.con(:,16).*a.con(:,16)
# Vref
a.dat(:,6) = Vc
# iqr max & min
...
# rotor speed
  ...
  while max(abs(inc)) > 1e-8
    ...
    if iter > 20
      fm_print([' * * Initialization of doubly fed ind. gen. #', ...
               num2str(i),' failed.'])
      check = 0
      break
...
if not check
  fm_print('Doubly fed induction generators cannot be properly initialized.')
else
  fm_print(['Initialization of doubly fed induction generators completed.'])
