...
# Source: third-party/psat/@DFclass\Fxcall.m  (upstream PSAT, GPL-2.0+)
...
iomega = 1./(omega_m+(not a.u))
Vwrate = getvw(Wind,a.wind)
Vw = vw.*Vwrate
dPwdx = windpower(a,rho,Vw,A,R,not a.u+omega_m,theta_p,2)./Settings.mva/1e6
Pw = ng.*windpower(a,rho,Vw,A,R,not a.u+omega_m,theta_p,1)/Settings.mva/1e6
Tm = Pw.*iomega
iqrsign = np.ones((a.n,1))
w21 = 2*omega_m-1
idx = find(w21 <= 0 | w21 >= 1)
if not isempty(idx), iqrsign(idx) = -1; end
...
# mechanical equation
# -------------------
DAE.Fx = DAE.Fx + sparse(a.omega_m,a.omega_m,-(not a.u)+(ng.*dPwdx(:,1)-Tm).*i2Hm.*iomega,DAE.n,DAE.n)
...
if not isempty(idx), tspo(idx) = 0; end
iqr_max = a.dat(:,8)
iqr_min = a.dat(:,9)
