...
# Source: third-party/psat/fm_idx.m  (upstream PSAT, GPL-2.0+)
...
# state variables
  if DAE.n
for j in range(1, len(Varname.fnamex)+1):
...
# Q
  ...
  if DAE.m > 2*Bus.n
for j in range(1, len(Varname.fnamey)+1):
...
# S_ji
  ...
  Varname.nvars = len(Varname.uvars)
# plot variables
