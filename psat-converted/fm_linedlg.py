...
# Source: third-party/psat/fm_linedlg.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
def fm_linedlg(varargin):
...
case 1
    ...
    if isempty(arg1)  or  ishandle(arg1(1))  or  isa(arg1(1), 'scribehandle')
...
# might be any selected object
    ...
    fm_print(['Unable to edit line properties: invalid line handles'])
    return
...
# ArrowVector(end+1) = aObj;
    if isempty(LineVector)
        ...
        IndLine = [1:len(LineVector)]
    HG = [LineVector ArrowVector]

catch
    set(fig,'Pointer',oldPointer)
    fm_print(['Unable to open line properties dialog. ' ...
...
if isequal(len(ArrowVector),len(HG)),
    MarkerEnable='off'
else
    MarkerEnable='on'

#---Get all object data
for ctHG in range(1, len(HG)+1):
...
# Find Common Marker
    markerVal = 1
    if not isempty(IndLine),
        CommonMarker = unique({GetData(IndLine).Marker})
        if len(CommonMarker)==1,
...
try
    ...
    fm_print(['Unable to display help for Line Properties:' ...
            sprintf('\n') lasterr ],2)

function localData = button(selection, localData)
switch selection
case 'close'
for ct in range(1, len(localData.HG)+1):
...
fm_print([fieldName ' field requires a single positive numeric input'],2)
...
#---Only change Linestyle if not set to Current
    if not strcmp(getval(f,'LineStyle'),'Current'),
        lineSettings.LineStyle = getval(f,'LineStyle')
        if not localData.CommonStyle,
...
#---Only change Marker if not set to Current
    if not strcmp(getval(f,'Marker'),'Current'),
        lineSettings.Marker= getval(f,'Marker')
        if not localData.CommonMarker,
...
for ctHG in range(1, len(HG)+1):
    ...
    if get(Hdl_legend,'Value')
        ...
        n_var = len(get(Hdl_listplot,'String'))
        tipoplot = get(Hdl_tipoplot,'Value')

        if tipoplot == 4  or  tipoplot == 5

            h = findobj(hdl,'Type','line')

for i in range(0, n_var-1+1):
...
#hmarker = plot((xdata(2)-xdata(1))/1.2,ydata(1));
    ...
    fm_print(lasterr,2)
    fm_print('Unable to set line properties.',2)


function LNoLineError
fm_print(['No lines are selected.  Click on an line to select it.'],2)