...
# Source: third-party/psat/fm_gamsfig.m  (upstream PSAT, GPL-2.0+)
...
if nargin
  switch varargin{1}
   ...
   case 'method'
    ...
    if GAMS.method == 4  or  GAMS.method == 6  or  GAMS.method == 7
      set(hdl,'Enable','on')
    else
      set(hdl,'Enable','off')
   ...
   case 'libinclude'
    ...
    GAMS.libinclude = not GAMS.libinclude
    if GAMS.libinclude,
      set(gcbo,'Checked','on')
    else
      set(gcbo,'Checked','off')
   ...
   case 'view'
    ...
    if not rem(GAMS.type,2)  and  strcmp(file,'fm_gams.lst')
      file = 'fm_gams2.lst'
    if GAMS.method == 7  and  strcmp(file,'fm_gams.lst')
      file = 'fm_cong.lst'
   ...
   case 'omega'
    ...
    catch, fm_print(lasterr,2), set(hdl_omeg,'String',GAMS.omega_s), return, end
    if max(GAMSomega) > 1  or  min(GAMSomega) < 0
        fm_print('The weighting factor must be within the range [0,1].',2)
        set(hdl_omeg,'String',GAMS.omega_s), return
    [ao,bo] = size(GAMSomega)
    if ao > 1  and  bo > 1
        fm_print('The weighting factor must be a vector.',2)
        set(hdl_omeg,'String',GAMS.omega_s), return
    GAMS.omega = GAMSomega
    GAMS.omega_s = s_omega
    fm_print(['Parameter "GAMS.omega" set to "',GAMS.omega_s,'".'],1)
   ...
   case 'lmin'
    ...
    catch, fm_print(lasterr,2), set(hdl_lmax,'String',GAMS.lmin_s), return, end
    if max(GAMSlmin) > GAMS.lmax
        fm_print('lambda_min must be lower than lambda_max.',2)
        set(hdl_lmin,'String',GAMS.lmin_s), return,
    [al,bl] = size(GAMS.lmin)
    if al > 1  and  bl > 1
        fm_print('The loading parameter must be a vector.',2)
        set(hdl_lmin,'String',GAMS.lmin_s), return
    GAMS.lmin = GAMSlmin
    GAMS.lmin_s = s_lmin
    fm_print(['Parameter "GAMS.lmin" set to "',GAMS.lmin_s,'".'],1)
...
h1 = uicontrol('Parent',h0, ...
               ...
               'Tag','ListboxLines', ...
               'Value',GAMS.line+1)
if GAMS.method != 4  and  GAMS.method != 6  and  GAMS.method != 7
  set(h1,'Enable','off')
...
h1 = uicontrol('Parent',h0, ...
               ...
               'Tag','PopupMenuMethod', ...
               'Value',GAMS.method)
if GAMS.type == 2  or  GAMS.type == 4
  set(h1,'String',metodi([1,2]))
if GAMS.method == 4  and  GAMS.type == 3
  set(h1,'Enable','off')
h1 = uicontrol('Parent',h0, ...
               'Units', 'normalized', ...
               'BackgroundColor',Theme.color04, ...
