...
# Source: third-party/psat/@AVclass\setx0.m  (upstream PSAT, GPL-2.0+)
...
# signal V/V0 & initial voltage v0
  z = a.con(ty3,9)
  idx = find(z)
  if not isempty(idx), a.con(ty3(idx),9) = vg(ty3(idx)); end
...
if not check
  fm_print('Automatic Voltage Regulators cannot be properly initialized.')
else
  fm_print('Initialization of Automatic Voltage Regulators completed.')
