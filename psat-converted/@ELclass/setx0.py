# ------------------------------------------------------------------
# AUTO-CONVERTED FROM MATLAB BY tools/matlab_to_python.py
# Source: third-party/psat/@ELclass\setx0.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
function a = setx0(a)

global DAE PQ Bus

if not a.n, return, end

# parameter initialization

# dat:
#  col #1: P0
#  col #2: Q0
#  col #3: V0

a.dat(:,3) = DAE.y(a.vbus)

for k in range(1, a.n+1):
  idx = findbus(PQ,a.bus(k))
  if isempty(idx)
    fm_print(['No PQ load found for initializing Exponential ', ...
             'Recovery Load at bus ',Bus.names{a.bus(k)}])
  else
    a.dat(k,1) = a.u(k)*PQ.P0(idx)*a.con(k,3)/100
    a.dat(k,2) = a.u(k)*PQ.Q0(idx)*a.con(k,4)/100
    PQ = pqsub(PQ,idx,a.dat(k,1),a.dat(k,2))
    PQ = remove(PQ,idx,'zero')

# state variable initialization
DAE.x(a.xp) = 0
DAE.x(a.xq) = 0

# message
fm_print('Initialization of Exponential Recovery Loads completed.')

