...
# Source: third-party/psat/fm_pareto.m  (upstream PSAT, GPL-2.0+)
...
fm_print('  ')
fm_print('PARETO SET COMPUTATIONS')
...
Varout.idx = [1:len(Varname.fvars)]
...
Varout.vars = np.zeros((ntot,n_y+Bus.n+4*Line.n+1))
OPF.wp = OPF.omega
OPF.show = 0
OPF.conv = 0
idxrho = np.zeros((ntot,1))
...
for j in range(1, ntot+1):
  ...
  if OPF.conv == 1
    ...
    fm_print(['Current lambda = ', num2str(OPF.lmax)])
    OPF.w = 0.00001
    eval(OPF.fun)
...
if not isempty(find(idxrho))
for i in range(2, len(idxrho)+1):
    if idxrho(i) != 0  and  idxrho(i) != len(idxrho)
...
if ishandle(Fig.main)
  ...
  if not get(Fig.main,'UserData'),
    fm_print(['Pareto Set Computation interrupted.'],1),
    return

fm_print(['Pareto Set Computation completed in ', ...
