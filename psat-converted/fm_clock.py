...
# Source: third-party/psat/fm_clock.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
import numpy as np

def fm_clock(varargin):
...
if nargin
  switch varargin{1}
   case 'keyesc'
    ...
    if (double(tasto) == 13  or  double(tasto) == 27),
      delete(gcf)
...
h = text(0,0.775*sin(np.pi/6),'PSAT')
set(h, ...
    'FontName',fontname, ...
    'FontSize',10, ...
    'FontAngle','oblique', ...
...
t1 = np.pi/4:0.01:3*np.pi/4
t2 = 3*np.pi/4:-0.01:np.pi/4
hx = 0.05+1.2*(cos(np.pi/4)+[cos(t1), cos(t2)])/3
hy = 0.07*[sin(t1)-sin(np.pi/4),sin(np.pi/4)-sin(t2)]
mx = 0.05+2*(cos(np.pi/4)+[cos(t1), cos(t2)])/3
my = 0.05*[sin(t1)-sin(np.pi/4),sin(np.pi/4)-sin(t2)]

s = np.pi/2-np.pi/30*a(6)
b = a(6)/60
if b == 1; b = 0; end
m = np.pi/2-np.pi/30*(a(5)+b)
c = a(5)/60
if c == 1; c = 0; end
o = np.pi/2-np.pi/6*(a(4)+c)
...
hs2 = plot([0.05*cos(np.pi+s) 0.2*cos(np.pi+s)], ...
           [0.05*sin(np.pi+s) 0.2*sin(np.pi+s)])
...
while 1
  ...
  try
    ...
    set(hs2, ...
	'Xdata',[0.05*cos(np.pi+s) 0.2*cos(np.pi+s)], ...
	'Ydata',[0.05*sin(np.pi+s) 0.2*sin(np.pi+s)])
  catch
