...
# Source: third-party/psat/fm_spf.m  (upstream PSAT, GPL-2.0+)
...
fm_disp
fm_print('Newton-Raphson Method for Power Flow Computation')
if Settings.show
...
# create the FM_CALL FUNCTION
  if Settings.show, fm_print('Writing file "fm_call" ...',1), end
  fm_wcall
  fm_dynlf; #  indicization of components used in power flow computations

# memory allocation for equations and Jacobians
DAE.g = np.ones((DAE.m,1))
DAE.Gy = sparse(DAE.m,DAE.m)
if (DAE.n!=0)
  DAE.f = np.ones((DAE.n,1); #  differential equations)
  DAE.x = np.ones((DAE.n,1); #  state variables)
  fm_xfirst
...
ncload = getnum(Mn) + getnum(Pl) + getnum(Lines) + (not nodyn)
if (ncload  or  Settings.distrsw)  and  (PFmethod == 2  or  PFmethod == 3)
  if ncload
    fm_print('Fast Decoupled PF cannot be used with dynamic components')
  if Settings.distrsw
    fm_print('Fast Decoupled PF cannot be used with distributed slack bus model')
  PFmethod = 1; #  force using standard NR method

switch PFmethod
 case 1, fm_print('PF solver: Newton-Raphson method')
 case 2, fm_print('PF solver: XB fast decoupled method')
 case 3, fm_print('PF solver: BX fast decoupled method')
 case 4, fm_print('PF solver: Runge-Kutta method')
 case 5, fm_print('PF solver: Iwamoto method')
 case 6, fm_print('PF solver: Robust power flow method')
 case 7, fm_print('PF solver: DC power flow')
...
if Settings.distrsw
  ...
  DAE.kg = 0
  if not swgamma(SW,'sum')
    fm_print('Slack buses have zero power loss participation factor.')
    fm_print('Single slack bus model will be used')
    Setting.distrsw = 0
  if totp(SW) == 0
    SW = setpg(totp(PQ)-totp(PV),1)
    fm_print('Slack buses have zero generated power.')
    fm_print('P_slack = sum(P_load)-sum(P_gen) will be used.')
    fm_print('Only PQ loads and PV generators are used.')
    fm_print('If there are convergence problems, use the single slack bus model.')
  Gkcall(SW)
  Gkcall(PV)

switch Settings.distrsw
 case 1
  fm_print('Distributed slack bus model')
 case 0
  fm_print('Single slack bus model')
...
while (Settings.error > tol)  and  (iteration <= iter_max)  and  (alfa > 1e-5)
  ...
  switch PFmethod
   ...
   case {2,3} #  Fast Decoupled Power Flow
    ...
    if not iteration #  initialize variables
      Line = build_b(Line)
      no_sw = Bus.a
      no_sw(getbus(SW)) = []
      no_swv = no_sw + Bus.n
...
# P-theta
    ...
    fm_call('fdpf')
    normP = np.linalg.norm(DAE.g,inf)

# Q-V
    dV = -(Upp\(Lpp\(Ppp*(DAE.g(no_g)./DAE.y(no_g)))))
    DAE.y(no_g) = DAE.y(no_g)+dV
    fm_call('fdpf')
    normQ = np.linalg.norm(DAE.g,inf)

    inc = [normP; normQ]

# recompute Bpp if some PV bus has been switched to PQ bus
    if Settings.pv2pq  and  strmatch('Switch',History.text{end})
      fm_print('Recomputing Bpp matrix for Fast Decoupled PF')
      no_g = Bus.a
      no_g([getbus(SW); getbus(PV)]) = []
      Bpp = Line.Bpp(no_g,no_g)
      [Lpp, Upp, Ppp] = lu(Bpp)
...
# Cardan's formula
   ...
   case 6 #  simple robust power flow method
    ...
    if Settings.error > 1.5*err_max_old  and  iteration
      robust = 0.25*robust
      if robust < tol
        fm_print('The otpimal multiplier is too small.')
  ...
  if Settings.error < 1e-2  and  PFmethod > 3  and  Settings.switch2nr
    fm_print('Switch to standard Newton-Raphson method.')
    PFmethod = 1

  fm_status('pf','update',[iteration, Settings.error],iteration)

  if Settings.show
    if Settings.error == np.inf, Settings.error = 1e3; end
    fm_print(['Iteration = ', num2str(iteration), ...
...
# stop if the error increases too much
  if iteration > 4
    if err_vec(iteration) > 1000*err_vec(1)
      fm_print('The error is increasing too much.')
      fm_print('Convergence is likely not reachable.')
      convergence = 0
      break
...
if iteration > iter_max
  fm_print(['Reached maximum number of iteration of NR routine without ' ...
           'convergence'],2)
  convergence = 0
...
m_diff = DAE.m-m_old
if m_diff
  DAE.y = [DAE.y;np.zeros((m_diff,1)])
  DAE.g = [DAE.g;np.zeros((m_diff,1)])
  DAE.Gy = sparse(DAE.m,DAE.m)

# rebuild state variables and vectors
if DAE.n != 0
  DAE.f = [DAE.f; np.ones((DAE.n-DAE.npf,1)];  #  differential equations)
  DAE.x = [DAE.x; np.ones((DAE.n-DAE.npf,1)];  #  state variables)
...
if Settings.static
  fm_print('* * * Dynamic components are not initialized * * *')
if convergence
  Settings.init = 1
else
  Settings.init = -1
...
fm_print(['Power Flow completed in ',num2str(Settings.lftime),' s'])
if Settings.showlf == 1  or  ishandle(Fig.stat)
  fm_stat
else
...
if isempty(Settings.t0)  and  ishandle(Fig.main)
  hdl = findobj(Fig.main,'Tag','EditText3')
  Settings.t0 = str2num(get(hdl,'String'))

if not Settings.locksnap  and  Bus.n <= 5000  and  DAE.n < 5000
  Snapshot = struct( ...
  ...
  if Bus.n > 5000
    fm_print(['Snapshots are disabled for networks with more than 5000 ' ...
             'buses.'])
  if DAE.n > 5000
    fm_print(['Snapshots are disabled for networks with more than 5000 ' ...
             'state variables.'])
