...
# Source: third-party/psat/fm_dump.m  (upstream PSAT, GPL-2.0+)
...
dump_data(fid, SW, 'SW')
dump_data(fid, PV, 'PV')
for i in range(1, (len(Comp.names)-2)+1):
...
fclose(fid)
fm_print(['Data dumped to file <', filename ,'>'])

function dump_data(fid, var, name)

if not var.n, return, end
fprintf(fid, '%s.con = [ ...\n', name)
