...
# Source: third-party/psat/fm_pmutry.m  (upstream PSAT, GPL-2.0+)
...
for j in range(1, len(connes)+1):
    if len(find(Line.Y(connes(j),:))) == 2  and  ...
...
for ijk in range(1, test_len-1+1):
    ...
    linee(I_idx(:,2),[1 2]) = np.zeros((len(I_idx(:,1)),2))
...
for jjj in range(1, num_nodi+1):
    ...
    nodi_ord(nodi_el) = []
    if ishandle(Fig.pmu)
      set(hdl_nob,'String',int2str(Bus.n-len(nodi_ord)))
      drawnow
...
if test3_idx > 0  and  (test_len-len(pmu_test3)) < min_pmu
  ...
  pmu_status = [np.zeros((len(pmu_test3),1); pmu_status])

  if ishandle(Fig.pmu)
    set(hdl_pmu,'String',int2str(test_len-1))
    drawnow

elseif test3_idx > 0  and  (test_len-len(pmu_test3)) == min_pmu

  pmu_test_new = pmu_test
  pmu_test_new(idx4) = []
  if ishandle(Fig.pmu)
    set(hdl_pmu,'String',int2str(test_len-len(pmu_test3)))
    drawnow
  linee = [Line.fr, Line.to]
  I_idx = []
  nodi = []
...
for ijk in range(1, len(pmu_test_new)+1):
    i_idx = fm_iidx(pmu_test_new(ijk),linee)
    I_idx = [I_idx; i_idx]
    nodi_oss = [i_idx(:,4);pmu_test_new(ijk)]
    nodi = [nodi; nodi_oss]
...
for jjj in range(1, num_nodi+1):
  ...
  if len(pmu_test2{sel_pmu}) > min_pmu
    pmu_test2(sel_pmu) = []
    I_idx_test(sel_pmu) = []
    pseudi_test(sel_pmu) = []
    pmu_status(sel_pmu) = []
