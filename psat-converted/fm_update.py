...
# Source: third-party/psat/fm_update.m  (upstream PSAT, GPL-2.0+)
...
if nargin
  switch varargin{1}
   ...
   case 'author'
    ...
    if not exist('Fig'), Fig.update = -1, end
    if not ishandle(Fig.update)
       Fig.update = findobj(get(0,'Children'),'FileName','fm_update')
    poscur = get(Fig.update,'CurrentPoint')
    if poscur(1) > 0.0  and  poscur(2) > 0.0  and  poscur(1) < 0.05  and  poscur(2) < 0.05
        fm_author(3280)
