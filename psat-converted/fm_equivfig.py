...
# Source: third-party/psat/fm_equivfig.m  (upstream PSAT, GPL-2.0+)
...
if nargin
  ...
  switch varargin{1}
   ...
   case 'run'
    ...
    if EQUIV.bus_selection == 4  and  isempty(EQUIV.custom_file)
      fm_print('A bus list file must be defined.',2)
      return
    check = fm_equiv
    if check
      set(ht,'String','Equivalencing procedure completed.')
      fm_print('Equivalencing procedure completed.')
    else
      set(ht,'String','Equivalencing procedure failed.')
      fm_print('Equivalencing procedure failed.',2)
...
# adjusting region edit box
   ...
   case 'setbuslist'
    ...
    if ishandle(Fig.dir)
      ...
      set(hdl,'Enbale','inactive','Value',len(get(hdl,'String')))
   ...
   case 'createlist'
    ...
    uiwait(fm_busfig)
    a = dir([filedata,'.lst'])
    if not isempty(a)  and  isempty(EQUIV.custom_file)
      if a.date != bdate
...
if not isempty(EQUIV.custom_path)
