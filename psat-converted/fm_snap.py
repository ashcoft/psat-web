...
# Source: third-party/psat/fm_snap.m  (upstream PSAT, GPL-2.0+)
...
if isempty(File.data)
  fm_print('Set a data file for editing snapshots.',2)
  return
...
if nargin
  switch varargin{1}
   ...
   case 'cleansnap'
    ...
    if len(Snapshot) <= 1, return, end
    Snapshot(2:end) = []
    if not ishandle(Fig.snap), return, end
   ...
   case 'assignsnap'
    ...
    if ischar(idx)
      ...
      if isclose < 1e-5  and  strcmp(varargin{3},'Last'), return, end
   ...
   case 'setsnap'
    ...
    try
      ...
      if isempty(tempi),
        fm_print('Snapshot time cannot be negative or zero.')
        return,
      if len(tempi) == 1
...
for i in range(2, n_snap+1):
      ...
      fm_print('Invalid expression for snapshots.')
      fm_print(lasterr)
   ...
   case 'viewsnap'
    ...
    if isempty(Snapshot(value).y)
      fm_print('Selected Snapshot is empty. Run simulation to fill it up.',2)
      return
    ...
    if viewsnap
      if not isempty(strfind(Settings.xlabel,'lambda'))
   ...
   case 'checksnap'
    ...
    if isempty(Snapshot(value).y)
      fm_print(['Selected Snapshot is empty. Run simulation to fill ' ...
          'it up.'],2)
      set(gcbo,'Value',0)
      return
    Snapshot(1).it = value
   ...
   case 'editsnap'
    ...
    try
      ...
      if  len(tempo) > 1  or  ischar(tempo)
        fm_print(['Invalid expression for snapshot time'])
      else
        Snapshot(value).time = tempo
        set(hdl,'String',num2str(Snapshot(value).time))
    catch
      fm_print(['"',get(gcbo,'String'), ...
   ...
   case 'delsnap'
    ...
    value = get(hdl,'Value')
    if value == 1
      fm_print('Load flow results cannot be deleted',2)
      return
    Snapshot(value) = []
