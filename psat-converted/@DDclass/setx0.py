...
# Source: third-party/psat/@DDclass\setx0.m  (upstream PSAT, GPL-2.0+)
...
# Constants
# etaGB*4*R*pi*f/p
a.dat(:,1) = 4*np.pi*Settings.freq*a.con(:,17)./a.con(:,18).*a.con(:,20)
# A
a.dat(:,2) = np.pi*a.con(:,17).*a.con(:,17)
...
# Vref
  ...
  if Pc(i)*Settings.mva < a.con(i,3)  and  Pc(i) > 0
  ...
  while max(abs(inc)) > 1e-10
    ...
    if iter > 40
      fm_print(['Initialization of direct drive syn. gen. #', num2str(i), ' failed.'])
      check = 0
      break
...
# wind power [MW]
  ...
  if Pc(i) < 0
    fm_print([' * * Turbine power is negative at bus <',Bus.names{a.bus(i)},'>.'])
    fm_print(['     Wind speed <',num2str(a.wind(i)),'> cannot be initilized.'])
...
# initial guess for wind speed
  ...
  while abs(eqnvw) > 1e-7
    if iter > 50
      ...
      fm_print([' * * Initialization of wind speed <', ...
               wspeed,'> failed (convergence problem).'])
      fm_print(['     Tip: Try increasing the nominal wind speed <',wspeed,'>.'])
      check = 0
      break
...
if not check
  fm_print('Direct drive synchronous generator cannot be properly initialized.')
else
  fm_print('Initialization of direct drive synchronous generators completed.')
