...
# Source: third-party/psat/fm_plotsel.m  (upstream PSAT, GPL-2.0+)
...
if isempty(File.data)
  fm_print('Set a data file before selecting plot variables.',2)
  return
...
if nargin
  switch varargin{1}
   ...
   case 'appendidx'
    ...
    if Settings.init == 0,
      fm_print('Run power flow before saving plot variable indexes.',2),
      return,
    if isempty(strfind(filedata,'(mdl)'))
      ...
      count = fprintf(fid,'   ];\n')
      count = fprintf(fid, '\nVarname.areas = [...\n')
      nidx = len(Varname.areas)
      count = fprintf(fid,'%5d; %5d; %5d; %5d; %5d; %5d; %5d;\n',Varname.areas)
      if rem(nidx,7) != 0,
        count = fprintf(fid,'\n')
      count = fprintf(fid,'   ];\n')
      count = fprintf(fid, '\nVarname.regions = [...\n')
      nidx = len(Varname.regions)
      count = fprintf(fid,'%5d; %5d; %5d; %5d; %5d; %5d; %5d;\n',Varname.regions)
      if rem(nidx,7) != 0,
        count = fprintf(fid,'\n')
      count = fprintf(fid,'   ];\n')
      fclose(fid)
      fm_print(['Plot variable indexes appended to file "',Path.data,File.data,'"'])
    else
...
set(h5,'Value',max(1,len(Varname.idx)-8))
...
if Varname.x  or  (Varname.y  and  not isempty(y))
  values = [x; DAE.n+y]
