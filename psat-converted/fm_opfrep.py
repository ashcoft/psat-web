...
# Source: third-party/psat/fm_opfrep.m  (upstream PSAT, GPL-2.0+)
...
if OPF.init == 0
  fm_print('Solve OPF before writing the OPF report.',2)

if OPF.init > 2
  fm_print('OPF report is not defined for the current OPF solution.',2)
...
if OPF.enflow
  ...
  Iijmax = 999*np.ones((Line.n,1))

if OPF.enreac
  [Qgmax,Qgmin] = fm_qlim('gen')
else
  Qgmin = -99*MVA*np.ones((n_gen,1))
  Qgmax =  99*MVA*np.ones((n_gen,1))
...
Cols{3,1} = ''
bindings = len(find(round(OPF.guess(n_s+1:2*n_s)*1e5)))
