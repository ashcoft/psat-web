...
# Source: third-party/psat/fm_opffig.m  (upstream PSAT, GPL-2.0+)
...
if nargin
  switch varargin{1}
   ...
   case 'vlimits'
    ...
    if not isempty(a)
      try
        ...
        fm_print('Maximum voltage limit couldn''t be changed')
        OPF.vmax = 1.2
      try
        OPF.vmin = str2num(a{2})
      catch
        OPF.vmin = 0.8
        fm_print('Minimum voltage limit couldn''t be changed')
    if isempty(OPF.vmax)
      fm_print('Maximum voltage limit couldn''t be changed')
      OPF.vmax = 1.2
    if isempty(OPF.vmin)
   ...
   case 'omega'
    ...
    catch, fm_print(lasterr,2), set(hdl_omeg,'String',OPF.omega_s), return, end
    if max(OPFomega) > 1  or  min(OPFomega) < 0
    ...
    OPF.omega = OPFomega
    OPF.omega_s = s_omega
    fm_print(['Parameter "OPF.omega" set to "',OPF.omega_s,'".'],1)
   ...
   case 'lmin'
    ...
    [al,bl] = size(OPFlmin)
    if al > 1  or  bl > 1
      fm_print('The loading parameter must be a scalar.',2)
      set(hdl_lmin,'String',OPF.lmin_s)
      return
    OPF.lmin = OPFlmin
    OPF.lmin_s = s_lmin
    fm_print(['Parameter "OPF.lmin" set to "',OPF.lmin_s,'".'],1)
