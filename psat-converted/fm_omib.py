...
# Source: third-party/psat/fm_omib.m  (upstream PSAT, GPL-2.0+)
...
while k < 200
  ...
  while max(abs(inc)) > tol
    ...
    x = x + inc
    h = h + 1
    print([h, x(1), x(2), inc(1), inc(2), f(1), f(2)])
    pause

  Pe = Pc + Pmax*sin(x(1)-d0)
  Pa = Pm - Pe

  if (Pe_old > Pm)  and  (Pe < Pm)  and  ((Pa-Pa_old) > 0)
    deltau = 0.5*(x(1) + xa(1)); #  instability condition
    omegau = 0.5*(x(2) + xa(2))
    tu = t - dt/2
    break

  if (Pa < 0)  and  (xa(2) > 0)  and  (x(2) < 0)
