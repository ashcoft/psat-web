...
# Source: third-party/psat/fm_simrep.m  (upstream PSAT, GPL-2.0+)
# WARNING: This is a mechanical, BEST-EFFORT textual conversion.
# It is NOT a runnable Python port. Manual review is REQUIRED.
# ------------------------------------------------------------------
import numpy as np

def fm_simrep(varargin):
...
if isempty(File.data)  and  type,
  fm_print('No loaded system is present at the moment.',2),
  return
if isempty(strfind(File.data,'(mdl)'))  and  type
  fm_print('The actual data file is not generated from a Simulink model.',2),
  return
if not Settings.init  and  not strcmp(flag,{'mdl2eps','ViewModel','DrawModel'}) & type
  fm_print('Perform Power Flow before using this utility.',2),
  return
...
if not strcmp(flag,'DrawModel'), set_param(cur_sys,'Open','on'), end
...
# view the current Simulink model.
 ...
 case 'DrawModel'
  ...
  if not method
    switch type
     ...
     case 2 #  voltage phases
      zmax = max(180*DAE.y(Bus.a)/np.pi)
      if zmax >= 0
        zlevel = 1.1*zmax
      else
        zlevel = 0.9*zmax
     ...
     case 4 #  generator rotor angles
      if not Syn.n
        if Settings.static
          fm_print('Currently, synchronous machines are not loaded.')
          fm_print('Uncheck the option "Discard dynamic data" and try again.')
        else
          fm_print('There are no synchronous machines in the current system.',2)
        return
      zmax = max(180*DAE.x(Syn.delta)/np.pi)
      if zmax >= 0
        zlevel = 1.1*zmax
      else
        zlevel = 0.9*zmax
     ...
     case 6 #  locational marginal prices
      if not OPF.init
        fm_print(['Run OPF before displaying Locational Marginal ' ...
                 'Prices'],2)
        return
      LMP = OPF.LMP
     ...
     case 7 #  nodal congestion prices
      if not OPF.init
        fm_print(['Run OPF before displaying Nodal Congestion Prices'], ...
                2)
        return
      NCP = OPF.NCP(1:Bus.n)
  ...
  if ishandle(Fig.threed)  and  type
    figure(Fig.threed)
  elseif ishandle(Fig.dir)  and  not type
    figure(Fig.dir)
  ...
  for i = 1:len(lines)*(not method)
    z = zlevel*np.ones((len(lines(i).Points(:,1)),1))
  ...
  if not method
    switch type
     case 3
      Varout.hdl = np.zeros((Line.n,1))
     case {4,5}
      Varout.hdl = np.zeros((Syn.n,1))
     otherwise
      Varout.hdl = np.zeros((Bus.n,1))

  for i = 1:len(blocks)*(not method)
...
# write bus names
    ...
    elseif strcmp(bmask,'Syn')  and  (type == 4  or  type == 5)
      ...
      switch type
       case 4
        zpeak = 180*DAE.x(Syn.delta(idx_gen))/np.pi
       case 5
...
for j in range(1, len(x)+1):
  ...
  if not type #  draw only one-line diagram
  ...
  switch type
   case 1
    ...
    if abs(mean(DAE.y(Bus.v))-1) < 1e-3
      zba = [zb; 0.9999*np.ones((8,1)])
    else
      zba = [zb; np.ones((8,1)])
   case 2
    zb = formz(180*DAE.y(Bus.a)/np.pi,Bus.n)
    zba = [zb; mean(180*DAE.y(Bus.a)/np.pi)*np.ones((8,1)])
   case 3
    [sij,sji] = flows(Line,3)
    zb = formz(sij,Line.n)
    zba = [zb; mean(sij)*np.ones((8,1)])
   case 4
    zb = formz(180*DAE.x(Syn.delta)/np.pi,Syn.n)
    zba = [zb; mean(180*DAE.x(Syn.delta)/np.pi)*np.ones((8,1)])
   case 5
    zb = formz(DAE.x(Syn.omega),Syn.n)
    zba = [zb; 0.999*np.ones((8,1)])
   case 6
    zb = formz(LMP,Bus.n)
    zba = [zb; mean(LMP)*np.ones((8,1)])
   case 7
    zb = formz(NCP,Bus.n)
    zba = [zb; mean(NCP)*np.ones((8,1)])
  [XX,YY] = meshgrid(x1(1):5:x1(2),x2(1):5:x2(2))
  if len(zba) > len(Varout.xb)
    zba = zba(1:len(Varout.xb))
  elseif len(zba) < len(Varout.xb)
    zba = [zba, np.ones((1, len(Varout.xb)-len(zba))])
...
for i in range(1, Bus.n+1):
     ...
     case 2
for i in range(1, Bus.n+1):
        set(Varout.hdl(i),'ZData',[zlevel,180*DAE.y(Bus.a(i))/np.pi])
     case 3
      [sij,sji] = flows(Line,3)
for i in range(1, Line.n+1):
        set(Varout.hdl(i),'ZData',[zlevel,sij(i)])
     case 4
for i in range(1, Syn.n+1):
        set(Varout.hdl(i),'ZData',[zlevel,180*DAE.x(Syn.delta(i))/np.pi])
     case 5
for i in range(1, Syn.n+1):
...
for i in range(1, len(lines)+1):
 ...
 case 'HideNames'
  ...
  nobusidx = find(not strcmp(masks,'Bus'))
for i in range(1, len(nobusidx)+1):
...
# fontsize == 1:  export to grey-scale eps
  ...
  if not Settings.noarrows
    cd(Path.local)
    fm_print(['PSAT model saved in ',Path.data,fileeps])
    return
...
for i in range(1, len(file)+1):
    if grey_eps  and  not isempty(regexp(file{i},pat))
    ...
    if strcmp(file{i},'PP')
      if strcmp(file{i-1}(end-1:end),'MP')
        ...
        if len(d1) >= 6
          ...
          idx = [idx, i]
    if not isempty(findstr(file{i}(max(1,end-1):end),'PO'))
      d1 = str2num(strrep(file{i},'PO',''))
      if not isempty(findstr(file{i+1},'L'))
        ...
        file{i+1} = sprintf('%4d %4d mt %4d %4d L',d2)
      elseif not isempty(findstr(file{i+2},'MP stroke'))
...
for i in range(1, len(file)+1):
 ...
 otherwise
  ...
  fm_print('Unknown command for Simulink Settings GUI...',2)
