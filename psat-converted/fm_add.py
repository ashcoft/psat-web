...
# Source: third-party/psat/fm_add.m  (upstream PSAT, GPL-2.0+)
...
s3 = ' variable list.'
s4 = 'in the '
if not isvarname(nome)
  fm_print([s1,nome,'" is not a valid Matlab variable.'])
  return
# check if variable is already used
if strmatch(nome,Algeb.name,s0)
  fm_print([s1,nome,s2,'as an algebraic variable.'])
  return
if strmatch(nome,Buses.name,s0)
  fm_print([s1,nome,s2,'as a bus variable.'])
  return
if strmatch(nome,Initl.name,s0)
  fm_print([s1,nome,s2,'as an initial variable.'])
  return
if strmatch(nome,Param.name,s0)
  fm_print([s1,nome,s2,'as a parameter.'])
  return
if strmatch(nome,Servc.name,s0),
  fm_print([s1,nome,s2,'as a service variable.'])
  return
if strmatch(nome,State.name,s0)
  fm_print([s1,nome,s2,'as a state variable.'])
  return
# check for variable consistency
if any(strmatch(nome,Servc.idx,s0))  and  ...
      not strcmp(list_box,'ListboxServiceVar')
  fm_print([s1,nome,s2,s4,'service',s3])
  return
if any(strmatch(nome,Algeb.idx,s0))  and  ...
      not strcmp(list_box,'ListboxAlgebraic')
  fm_print([s1,nome,s2,s4,'algebraic',s3])
  return
if any(strmatch(nome,Initl.idx,s0))  and  ...
      not strcmp(list_box,'ListboxInitial')
  fm_print([s1,nome,s2,s4,'initial',s3])
  return
if strcmp(nome,'null')  or  strcmp(nome,'None')
  fm_print([s1,nome,'" is a protected word.'])
  return
...
for i in range(1, numero+1):
    ...
    set(hdl(13),'String',Algeb.idx,'Value',len(Algeb.idx))

    Servc.idx = Servc.oldidx
    actualservice = len(Servc.idx)
for i in range(1, numero+1):
        Servc.idx{actualservice+i,1} = ['P', int2str(i)]
        Servc.idx{actualservice+numero+i,1} = ['Q', int2str(i)]
    set(hdl(14),'String',Servc.idx,'Value',len(Servc.idx))

    actualinit = len(Initl.idx)
    ...
    set(hdl(15),'String',Initl.idx,'Value',len(Initl.idx))
