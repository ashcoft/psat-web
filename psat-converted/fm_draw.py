...
# Source: third-party/psat/fm_draw.m  (upstream PSAT, GPL-2.0+)
...
# imresize is not available!
 case 'arrow'
  ...
  y = [-0.1 0 0.1 0 0]
 case 'circle'
  t = 0:0.05:2*np.pi+0.05
  x = cos(t)
  y = sin(t)
  return
 case 'semicircle'
  t = -np.pi/2:0.05:np.pi/2+0.05
  x = cos(t)
  y = sin(t)
  return
 case 'rounded'
  t = 0:0.05:np.pi/2+0.05
  a = 0.6
  k = 0.4
  xc = k*cos(t)
  yc = k*sin(t)
 ...
 case 'J'
  ...
  y = [-1-y1,-1,2,2,2,2]/4
 case 'G'
  t = np.pi/4:0.05:2*np.pi-np.pi/10+0.025
  x1 = cos(t)
  y1 = sin(t)
  x = [x1,0]
  y = [y1,-0.2934]
