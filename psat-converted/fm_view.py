...
# Source: third-party/psat/fm_view.m  (upstream PSAT, GPL-2.0+)
...
if isempty(Bus.con)
  fm_print('No loaded system. Matrix visualization cannot be run.',2)
  return
...
switch flag
 case 1
  if DAE.n > 0
    ...
    fm_print('Since no dynamic component is loaded, AC coincides with Jlfv.',2)
    matrice = DAE.Gy
  titolo = 'Complete System Matrix A_c'
  visual(matrice,titolo)
 case 2
  if DAE.n > 0
    matrice = DAE.Fx - DAE.Fy*np.linalg.inv(DAE.Gy)*DAE.Gx
  else
    fm_print('No dynamic component loaded.',2)
    return
 ...
 case 5
  if DAE.n > 0
    Fx_mod = DAE.Fx+diag(-1e-5*np.ones((DAE.n,1))
    matrice = DAE.Gy - DAE.Gx*np.linalg.inv(Fx_mod)*DAE.Fy
  else
    fm_print(['Since no dynamic component is loaded, ', ...
