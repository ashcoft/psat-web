...
# Source: third-party/psat/fm_opfm.m  (upstream PSAT, GPL-2.0+)
...
if not Supply.n,
  fm_print('Supply data have to be specified in order to run OPF routines',2)
  return
...
ro = np.zeros((n2,1);   #  Dual Variables [ro])
...
for i in range(1, len(a)+1):
    fm_print(['     Bus #', fvar(getidx(Bus,a(i)),4), ...
             ' -> ',fvar(Vbus(a(i)),8), ...
             ' > ', fvar(Vmax(a(i)),8)],1)
  fm_print('Optimization routine interrupted.',1)
  return
if not isempty(b),
  fm_print(['Min Voltage limit not respected at buses: ',num2str(b')],1)
for i in range(1, len(b)+1):
    fm_print(['     Bus #', fvar(getidx(Bus,b(i)),4), ...
             ' -> ',fvar(Vbus(b(i)),8), ...
             ' < ', fvar(Vmin(b(i)),8)],1)
  fm_print('Optimization routine interrupted.',1)
  return
...
# =======================================================================
  ...
  norma3 = np.linalg.norm(DAE.g,inf)
...
if (Settings.showlf  or  OPF.show)  and  clpsat.showopf
  ...
  OPF.report{2+(not noDem),1} = ['Bid Losses = ', ...
                      fvar(sum(Line.p)-Snapshot(1).Ploss,8), ' [p.u.]']
  OPF.report{3+(not noDem),1} = ['Total Losses = ', ...
  ...
  elseif ishandle(Fig.main)
    if not get(Fig.main,'UserData')
      ...
      fm_print(['IPM-OPF completed in ',num2str(toc),' s'],1)
  else
    fm_print(['IPM-OPF completed in ',num2str(toc),' s'],1)
