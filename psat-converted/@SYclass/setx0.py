...
# Source: third-party/psat/@SYclass\setx0.m  (upstream PSAT, GPL-2.0+)
...
for i in range(1, a.n+1):
  ...
  if n_new != n_old
    ...
    if len(idx) == 1
      if a.con(idx,22) != 1
        fm_print(['Warning: Active power ratio of ', ...
                 'generator #', ...
                 num2str(idx),' must be 1'])
        a.con(idx,22) = 1
      if a.con(idx,23) != 1
        fm_print(['Warning: Reactive power ratio of ', ...
                 'generator #', ...
                 num2str(idx),' must be 1'])
        a.con(idx,23) = 1
    elseif len(idx) > 1
      ...
      if abs(ratiop-1) > 1e-5
        fm_print(['Warning: The sum of active power ', ...
      ...
      if abs(ratioq-1) > 1e-5
        fm_print(['Warning: The sum of reactive power ', ...
                 'ratios of generators #', ...
                 num2str(idx'),' must be 1'])
...
Vdq = a.u.*V.*exp(-jay*(delta-np.pi/2))
Idq = a.u.*I.*exp(-jay*(delta-np.pi/2))
vd = real(Vdq)
vq = imag(Vdq)
a.Id = real(Idq)
a.Iq = imag(Idq)
...
for j in range(1, a.n+1):
  if not fm_rmgen(a.bus(j)*a.u(i)), check = 0; end

fm_print('Initialization of Synchronous Machines completed.')
